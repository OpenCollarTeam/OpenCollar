...
// This script has been modified slightly to work with the new Owners HUD 3.4

// The cool color text and tinting was unfortunatly removed for two reasons: =(
// A) The text was off the screen if positioned on top, or cut off of the button was close to the side.
// B) If the user was using a tinted hud/texture, the color tint would be modified.
// Id like to go back and add in a small/invis prim that changes position on placement for this.
// It could aso display other HUD messages.

// Changelog:
// 1) Removed: touch(integer total_number) completly
// 2) Script moved to root
// 3) llSetLinkColor(3 -changed to- llSetLinkColor(4
// 4) llSetLinkColor(4,<1,0,0>,ALL_SIDES);  -changed to- llSetLinkColor(4,<1,1,1>,ALL_SIDES);
// 5) llSetText -changed to- //llSetText
// 6) llSetLinkColor -changed to- //llSetLinkColor
// 7) checks if the victim is using a RLViewer instead of checking for a relay (see below)
// integer answerchannel = 1812221819;
// integer answerlisten;
// line 243 llOWnerSay (message) = found avis
// line 247 check for viewer verrsion instead of answer from relay
// increased listener timeout to 10 seconds (line 250)
// listen event changed accordingly to listen on answerchannel for answer of viewer
// line 292 llOwnerSay(msg); = found avis with viewer
...
// --- float ---
float range=20.0;  // as far as llSay carries
...
default
{
    ...
    {
        //debug("that comes back on RLVRSchannel: "+ " ,name: " + name+ " ,message: "+message);
    ...
    timer()
    {
        ...
        {
            llOwnerSay("No avis with RLV in range at this time. Try again in a bit.");
            llResetScript();
        }
        else
        {
