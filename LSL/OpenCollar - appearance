...
{
    string sPrompt = "Adjust the " + CTYPE + " rotation.";
...
{
    string sPrompt = "Adjust the " + CTYPE + " scale. It is based on the size the collar has on rezzing. You can change back to this size by using '100%'.\nCurrent size: " + (string)g_iScaleFactor + "%\n\nATTENTION! May break the design of collars. Make a copy of the collar before using!";
...
DoMenu(key kAv, integer iAuth)
{
    ...
    if (g_iAppLock)
    {
        sPrompt = "The appearance of the " + CTYPE + " has be locked. To modified it a owner has to unlock it.";
        lMyButtons = [TICKED + APPLOCK];
    }
    else
    {
        sPrompt = "Which aspect of the appearance would you like to modify? Owners can lock the appearance of the " + CTYPE + ", so it cannot be changed at all.\n";
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        else if (iNum >= COMMAND_OWNER && iNum <= COMMAND_WEARER)
        {
            ...
            if (sCommand == "menu" && llGetSubString(sStr, 5, -1) == g_sSubMenu)
            {
                ...
                {
                    Notify(kID,"You are not allowed to change the " + CTYPE + " appearance.", FALSE);
            ...
            else if (sStr == "rotation")
            {
                if (kID!=g_kWearer && iNum!=COMMAND_OWNER)
                {
                    Notify(kID,"You are not allowed to change the " + CTYPE + " rotation.", FALSE);
                }
                else if (g_iAppLock)
                {
            ...
            else if (sStr == "position")
            {
                if (kID!=g_kWearer && iNum!=COMMAND_OWNER)
                {
                    Notify(kID,"You are not allowed to change the " + CTYPE + " position.", FALSE);
                }
                else if (g_iAppLock)
                {
                    Notify(kID,"The appearance of the " + CTYPE + " is locked. You cannot access this menu now!", FALSE);
            ...
            else if (sStr == "size")
            {
                if (kID!=g_kWearer && iNum!=COMMAND_OWNER)
                {
                    Notify(kID,"You are not allowed to change the " + CTYPE + " size.", FALSE);
                }
                else if (g_iAppLock)
                {
                    Notify(kID,"The appearance of the " + CTYPE + " is locked. You cannot access this menu now!", FALSE);
                    ...
                    else
                    {
                        ...
                        if (iMenuCommand != -1)
                        {
                            ...
                            if (iSizeFactor == -1000)
                            {
                                ...
                                {
                                    Notify(kAv, "The " + CTYPE + " is already at rez size, resizing canceled.", FALSE); 
                                }
                                else
                                {
