...
SetInstallmode(string type)
{
    ...
    string newstatus;
    //user clicked Full or Minimal.. set Bundle Status
    if(type == "Full")
    {
        newstatus = "INSTALL";
    }
...
GiveMethodMenu()
{
    string prompt = "How you want the Setup?";
    prompt += "\nChose Your Install method.";
    prompt += "\nCurrent method chosen : \""+INSTALL_METHOD+"\"\n";
    prompt += "\nFull: will Update/Install anything.";
    prompt += "\n((Exept things who needs to be removed))";
    prompt += "\nDefault: will Update/Intall/Remove as set by default files.";
    prompt += "\nMinimal: will Remove anything exept the Core Scripts.";
    prompt += "\nCustom: will open a new menu where you can chose what to do.";
    list choices = ["Full","Default","Minimal","Custom"];
    integer pointer = llListFindList(choices,[INSTALL_METHOD]);
    choices = llDeleteSubList(choices,pointer,pointer);
    kDialogID = Dialog(llGetOwner(), prompt + "\n", choices, ["START"],0);
}
