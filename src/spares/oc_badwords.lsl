...
integer g_bIsEnabled = FALSE;

integer g_bHasSworn = FALSE;
...
ListenControl() {
    if(g_bIsEnabled && llGetListLength(g_lBadWords)) {
        g_iListenerHandle = llListen(0, "", g_kWearer, "");
    } else {
...
string WordPrompt() {
    ...
    if (!iLength) {
        sPrompt = "%WEARERNAME% is not forbidden from saying anything.";
    } else if (iLength == 1) {
        ...
        if (sCommand == "add") { // Support owner adding words
            ...
            if (llGetListLength(lNewBadWords)) {
                while (llGetListLength(lNewBadWords)) {
                    ...
                    } else if (llListFindList(g_lBadWords, [sNewWord]) == -1) {
                        g_lBadWords += sNewWord;
                    }
            ...
            if (llGetListLength(lParams) > 2) {
                ...
                if(sName == "Default") {
                    if (g_bDefaultAnim) {
                        ...
                        sName = llList2String(g_lAnims, 0);
                    }
                }

                if (~llListFindList(g_lAnims, [sName]) || g_bDefaultAnim) {
                    ...
                    llMessageLinked(LINK_DIALOG, NOTIFY, "0" + " is not a valid animation name.", kID);
                }
            ...
            if (llGetListLength(lParams) > 2){
                ...
                string sName = llStringTrim(llGetSubString(sStr, iPos + 2, -1), STRING_TRIM);
                if (sName == "silent") {
                    llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "Punishment will be silent.", kID);
            ...
            if (llGetListLength(lNewBadWords)) {
                ...
                if (g_lBadWords) {
                    Dialog(kID, "Select a badword to remove or clear them all.", g_lBadWords, ["Clear", "BACK"], 0, iNum, "BadwordsRemove");
                } else {
...
default {
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
        ...
        if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) {
        ...
        } else if (iNum == ANIM_LIST_RESPONSE) {
            ParseAnimList(sStr);
            ...
            if (llGetSubString(sToken, 0, i) == g_sSettingToken) {
                ...
                if (sToken == "on") g_bIsEnabled = (integer)sValue;
            ...
            if (~iMenuIndex) {
                ...
                if (sMenu == "BadwordsMenu") {
                    if (sMessage == "BACK") {
                        llMessageLinked(LINK_ROOT, iAuth, "menu apps", kAv);
                    } else {
                        UserCommand(iAuth, "badwords " + sMessage, kAv, TRUE);
                    }
                } else if (sMenu == "BadwordsAdd") {
                    if (sMessage != " ") {
                        UserCommand(iAuth, "badwords add " + sMessage, kAv, TRUE);
                    } else {
                        MenuBadwords(kAv, iAuth);
                    }
                } else if (sMenu == "BadwordsRemove") {
                    if (sMessage == "BACK") {
                        MenuBadwords(kAv,iAuth);
                    } else if (sMessage == "Clear") {
                        UserCommand(iAuth, "badwords clear", kAv, TRUE);
                    } else if (sMessage) {
                        UserCommand(iAuth, "badwords remove " + sMessage, kAv, TRUE);
                    } else {
                        MenuBadwords(kAv,iAuth);
                    }
                    ...
                    if (sMessage == "BACK") {
                        ...
                        UserCommand(iAuth, "badwords animation " + sMessage, kAv, TRUE);
                    }
                } else if (sMenu == "BadwordsSound") {
                    if (sMessage == "BACK") {
                        MenuBadwords(kAv, iAuth);
                    } else {
                        UserCommand(iAuth, "badwords sound " + sMessage, kAv, TRUE);
                    }
                } else if (sMenu == "BadwordsPenance") {
                    if (sMessage) {
                        UserCommand(iAuth, "badwords penance " + sMessage, kAv, TRUE);
                    } else {
                        MenuBadwords(kAv, iAuth);
                    }
    ...
    listen(integer iChannel, string sName, key kID, string sMessage) {
        ...
        if ((~(integer)llSubStringIndex(llToLower(sMessage), llToLower(g_sPenance))) && g_bHasSworn) { // Stop sound
            if(g_sBadWordSound != g_sNoSound) {
                llStopSound();
            }
            // Stop anim
            ...
            g_bHasSworn = FALSE;
        } else if (~llSubStringIndex(sMessage, "rembadword")) {
            return; // Subs could theoretically circumvent this feature by sticking "rembadowrd" in all chat, but it doesn't seem likely to happen often
            ...
            while (llGetListLength(lWords)) {
                ...
                if (llListFindList(g_lBadWords, [sWord]) != -1) {
                    if(g_sBadWordSound != g_sNoSound) {
                        if (g_sBadWordSound == "Default") {
                            llLoopSound("4546cdc8-8682-6763-7d52-2c1e67e8257d", 1.0);
                        } else {
                            llLoopSound(g_sBadWordSound, 1.0);
                        }
                    }
