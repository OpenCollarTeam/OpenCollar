...
you will find a button named for the attachment. The menu inside here contains five buttons:
...
ADDON MENU - When activated the addon menu is opened on click, otherwise the main collar menu is opened. 
Note that there is no public access to the addon menu. 
SMART TARGET - (EXPERIMENTAL) If the leasher (dom) does not wear a collar or leashhandle then the leash chain 
goes to the avatar center (groin). With SMART TARGET the addon searches for objects attached to leasher's
ring fingers, wrists or hands to send the chain to.
...
Nikki Lacrima
    ...
    May 2026  -   Updated Lockmeister code that searches for leash target.
                  Added AddonMenu and SmartTarget, and save settings in LSD. 
...
integer g_iAddonMenuOnClick = FALSE;
integer g_iSmartLeashTarget = TRUE;
...
key g_kParticleTarget;             /* Prospective target for updated display chain, changed when probing for collar and handles */
key g_kLeashedTo;                  /* Avatar or object that wearer is leashed to, chain can go to child or attached prim */
key g_kLeashToPoint = NULL_KEY;    /* Actual prim that is used for displayed chain, remembered from latest call to StartParticles() */
...
list g_lAttachPoints = [
    ATTACH_RHAND_RING1, ATTACH_LHAND_RING1, 
    ATTACH_RLARM, ATTACH_LLARM, 
    ATTACH_RHAND, ATTACH_LHAND
];

StartParticles(key kParticleTarget) {
    /* Look for leash target instead of avatar center */
    if (kParticleTarget == NULL_KEY || g_iAddonActive == 0) return;
    if (kParticleTarget == g_kLeashedTo) {
        if (g_iSmartLeashTarget) {
            integer i;
            list lAttachments;
            key kTarget = NULL_KEY;
            for (i=0; i<llGetListLength(g_lAttachPoints) && (kTarget == NULL_KEY); i+=1) {
                lAttachments = llGetAttachedListFiltered(g_kLeashedTo, [FILTER_INCLUDE, llList2Integer(g_lAttachPoints,i)] );
                //llOwnerSay("  "+(string)llGetObjectDetails((key)llList2String(attachments,0), [OBJECT_NAME]) );
                kTarget = (key)llList2String(lAttachments,0);
                if (kTarget) kParticleTarget = kTarget; else kTarget = NULL_KEY;
            }
        }
        
    }
    //llWhisper(0, "start particles, prio "+(string)g_iCurrentPrio+" "+(string)kParticleTarget+" "+(string)llGetObjectDetails(kParticleTarget, [OBJECT_NAME]) );
    g_iDisplayedPrio = g_iCurrentPrio;
    //Debug(llList2CSV(g_lLeashPrims));
    StopParticles(FALSE);

    g_kLeashToPoint = kParticleTarget;
...
GetParticleSettings(integer iStartParticles) {
...
LockMeister_Handler(string sMessage, key id) {
    // Implementation of the LMV2 Protocol
    //llWhisper(0, "LM : "+sMessage);
    key kLMKey = (key)llGetSubString(sMessage,0,35);
    string sLMCommand = llGetSubString(sMessage,36,-1);
    list lLMCmd = llParseString2List(sMessage,["|"],[]);
    // Only handle messages for us or from g_kLeashedTo
...
//        llWhisper(0, "LM : "+(string)lLMCmd+"  "+sLMCommand);
        if (llGetListLength(lLMCmd) == 1) {
            // LM V1 dont overwrite V2
            if (sLMCommand == "handle ok") {
                AddLeashpointPrio(id, 1);
                llRegionSayTo(id,g_iChan_LOCKMEISTER,(string)g_kLeashedTo+"|LMV2|RequestPoint|handle");
            }
            if (sLMCommand == "collar ok") {
                AddLeashpointPrio(id, 3);
                llRegionSayTo(id,g_iChan_LOCKMEISTER,(string)g_kLeashedTo+"|LMV2|RequestPoint|collar");
            }
            else if  (sLMCommand == "handle detached") {
                ResetLeashpointPrio();
//                llSetTimerEvent(1); // Let timer handle the update
            ...
            if (llList2String(lLMCmd,3) == "handle") {
                AddLeashpointPrio(llList2Key(lLMCmd,4), 0);
            } else if (llList2String(lLMCmd,3) == "collar") {
                AddLeashpointPrio(llList2Key(lLMCmd,4), 2);
            }
        } // end of V2
...
Menu(key kID, integer iAuth) {
    string sPrompt = "\n"+g_sAddon+" collar addon.\n\nMake addon leashpoint override collar leashpoint.";
    sPrompt += "\nOpen addon menu (not collar) on click.";
    sPrompt += "\nLock addon.";
    list lButtons  = [llList2String(g_lCheckboxes, (g_iAddonActive>0))+" "+"Active",
                      llList2String(g_lCheckboxes, (g_iAddonMenuOnClick>0))+" "+"AddonMenu",
                      llList2String(g_lCheckboxes, (g_iAddonLocked>0))+" "+"Locked"];
    if(llGetLinkNumber()>1) { 
        sPrompt+="(Warning, this script is not in object root, and therefore Autohide will not work)";
    }
    else {
        sPrompt += "\nAutohide collar when wearing addon.";
        lButtons = lButtons+[llList2String(g_lCheckboxes, g_iAutohide)+" Autohide"];
    }
    sPrompt += "\nSmart Target: Try to find leash target from attachments when collar or leashhandle is missing.";
    lButtons = lButtons+[llList2String(g_lCheckboxes, (g_iSmartLeashTarget>0))+" "+"Smart Target"];
...
integer g_iAPILastRecvPong;
integer g_iAPILastSentPing;
integer g_iLastUpdateLeashpoint;

/* Leash point priorities, leash is attached to lowest active target priority.  
handleLMV2:0, handleLMV1:1, collarLMV2:2, collarLMV1:3, g_kLeashedTo:4 */
integer g_iDisplayedPrio = 4;
integer g_iCurrentPrio = 4;

/* Reset when new g_kLeashedTo */
ResetLeashpointPrio() {
...
/* Update after scan cycle and prepare for next */
UpdateLeashpoint() {
    /* update */
    StartParticles(g_kParticleTarget);
    /* reset and query */
    g_kParticleTarget = g_kLeashedTo;
    g_iCurrentPrio = 4;
    g_iLastUpdateLeashpoint = llGetUnixTime();
...
/* Add received targetkey, prio  */
AddLeashpointPrio(key kTarget, integer prio) {
    if (prio<g_iCurrentPrio) {
        //llWhisper(0, "AddLeashPoint, prio "+(string)prio+" "+(string)kTarget);
        g_kParticleTarget = kTarget;
        g_iCurrentPrio = prio;
        if (g_iCurrentPrio<=g_iDisplayedPrio) {
            StartParticles(g_kParticleTarget);
            g_iDisplayedPrio = g_iCurrentPrio;
            g_iLastUpdateLeashpoint = llGetUnixTime();
        }
...
default
{
    state_entry()
    {
        llOwnerSay("@detach=y");
        string sValue;
        sValue = llLinksetDataRead("leashaddon_active");
        if (sValue) g_iAddonActive = (integer)sValue; 
        sValue = llLinksetDataRead("leashaddon_menuonclick");
        if (sValue) g_iAddonMenuOnClick = (integer)sValue; 
        sValue = llLinksetDataRead("leashaddon_locked");
        if (sValue) g_iAddonLocked = (integer)sValue; 
        sValue = llLinksetDataRead("leashaddon_autohide");
        if (sValue) g_iAutohide = (integer)sValue;         
        if(llGetLinkNumber()>1) {
            llOwnerSay("Warning, "+llGetScriptName()+" is not in object root. This means autohide won't work and leashes won't auto-redraw to the collar if this is detached.");
            g_iAutohide = 0;
            llLinksetDataWrite("leashaddon_autohide","0");
        }
        sValue = llLinksetDataRead("leashaddon_smarttarget");
        if (sValue) g_iSmartLeashTarget = (integer)sValue;         
        ...
        llListen(g_iChan_LOCKMEISTER, "", "", "");
        Link("online", 0, "", llGetOwner()); // This is the signal to initiate communication between the addon and the collar
        g_iAPILastRecvPong = llGetUnixTime(); // Need to initialize this here in order to prevent resetting before we can receive our first pong
        llSetTimerEvent(60);
        FindLinkedPrims();
        GetParticleSettings(FALSE);
    ...
    attach(key id)
    {
        ...
        if(id)
        {
            ...
            llSetTimerEvent(60);
            g_iAPILastRecvPong = llGetUnixTime();
        }
        else
        {
            StopParticles(FALSE);
    ...
    changed(integer change)
    {
        ...
        {
            llLinksetDataDeleteFound("leashaddon_.*", "" );  // Clear Linkset Data
            llResetScript();
        }
    }
    ...
    timer()
    {
        if (llGetUnixTime() >= (g_iAPILastSentPing + 30))
        {
            g_iAPILastSentPing = llGetUnixTime();
            Link("ping", 0, "", g_kCollar);
        }

        if (llGetUnixTime() > (g_iAPILastRecvPong + (3 * 60)) && g_kCollar != NULL_KEY)
        {
            g_kCollar = NULL_KEY;
        ...
        // LockMeister leash handles, look for changed handles in order
        if (g_kLeashedTo!=NULL_KEY && g_iAddonActive && (llGetUnixTime()>= g_iLastUpdateLeashpoint+30)) 
        {
            UpdateLeashpoint();
        }
    }
    ...
    listen(integer channel, string name, key id, string msg){
        if (channel == API_CHANNEL) {
            ...
            // This signal, indicates the collar has approved the addon and that communication requests will be responded to if the requests are valid collar LMs.
                g_kCollar = id;
                g_iAPILastRecvPong = llGetUnixTime(); // Initial message should also count as a pong for timing reasons
...
//                                llSetTimerEvent(2);
                    ...
                    else if (iNum == LM_SETTING_RESPONSE)
                    {
                        ...
                        if (sToken ==  "particle") {
                            ...
                            GetParticleSettings(TRUE); // All setting changes are through LM_SETTING_RESPONSE
                            ...
                            if (sVar == "leashedto") {
                                ...
                                if ( ! llList2Integer(lValues,3)) {
                                    g_bLeasherIsAv = llList2Integer(lValues,2);
...
//                                        llSetTimerEvent(2);
                        ...
                        if(iPos>0)
                        {
                            ...
                            if (sMenu == "Menu~Main")
                            {
                                ...
                                else if (sMsg == llList2String(g_lCheckboxes, (g_iAddonActive>0))+" "+"Active")
                                {
                                    ...
                                    llLinksetDataWrite("leashaddon_active", (string)g_iAddonActive );
                                    UpdateLeashChain();
                                }
                                else if (sMsg == llList2String(g_lCheckboxes, (g_iAddonMenuOnClick>0))+" "+"AddonMenu")
                                {
                                    g_iAddonMenuOnClick = !g_iAddonMenuOnClick;
                                    llLinksetDataWrite("leashaddon_menuonclick", (string)g_iAddonMenuOnClick );
                                }
                                else if (sMsg == llList2String(g_lCheckboxes, (g_iAddonLocked>0))+" "+"Locked")
                                {
                                    g_iAddonLocked = !g_iAddonLocked;
                                    llLinksetDataWrite("leashaddon_locked", (string)g_iAddonLocked );
                                    if (g_iAddonLocked)
                                    {
                                        llOwnerSay("@detach=n");
                                    }
                                ...
                                else if (llGetSubString(sMsg,2,-1)=="Autohide")
                                {
                                    ...
                                    llLinksetDataWrite("leashaddon_autohide", (string)g_iAutohide );
                                    if (g_iAutohide)
                                    {
                                        Link("from_addon",0,"hide",llGetOwner());
                                        g_iSentHide=TRUE;
                                ...
                                else if (sMsg == llList2String(g_lCheckboxes, (g_iSmartLeashTarget>0))+" "+"Smart Target")
                                {
                                    g_iSmartLeashTarget = !g_iSmartLeashTarget;
                                    StartParticles(g_kParticleTarget);
                                    llLinksetDataWrite("leashaddon_smarttarget", (string)g_iSmartLeashTarget );
                                } 
        ...
        else if (channel == g_iChan_LOCKMEISTER) {
            LockMeister_Handler(msg, id); 
        } // end of LM_CHANNEL
    }
    
    touch_start(integer iNum){
        if (g_iAddonMenuOnClick) {
            Link("from_addon", 0, "menu "+llToLower(g_sAddon),  llDetectedKey(0));
        } else {
