...
integer g_bShock = FALSE;

list g_lAnims;
integer g_bDefaultAnim = FALSE;
...
Shock(integer iTime) {
    if (iTime > 0) {
        //llMessageLinked(LINK_DIALOG, NOTIFY, "1" + "%WEARERNAME% now shocked for " + (string)iTime + " seconds.", kID);
        llMessageLinked(LINK_DIALOG, SAY, "1" + "%WEARERNAME% now shocked for " + (string)iTime + " seconds.", "");
        if (g_sShockSound != g_sNoSound) {
            if (g_sShockSound == DEFAULT) {
                llLoopSound(g_sDefaultSound, 1.0);
            } else {
                llLoopSound(g_sShockSound, 1.0);
            }
        }

        g_bShock = TRUE;

        if (g_sShockAnim == DEFAULT) {
            llMessageLinked(LINK_ANIM, ANIM_START, g_sDefaultAnim, "");
        } else {
            llMessageLinked(LINK_ANIM, ANIM_START, g_sShockAnim, "");
        }

        llResetTime();
        llSetTimerEvent(iTime);
    } else if (g_bShock == TRUE) {
...
Stop() {
    if (g_bShock) {
        ...
        if (g_sShockAnim == DEFAULT) {
            llMessageLinked(LINK_ANIM, ANIM_STOP, g_sDefaultAnim, "");
        } else {
            llMessageLinked(LINK_ANIM, ANIM_STOP, g_sShockAnim, "");
        }
...
UserCommand(integer iAuth, string sStr, key kID, integer bRemenu) {
    ...
    if (sStr == "menu " + g_sSubMenu || sStr == "shocker") {
        if (iAuth == CMD_OWNER) {
            DialogShocker(kID, iAuth);
        } else {
            llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "Sorry, only the Owner can punish pet.", kID);
        }
    } else if (iAuth == CMD_OWNER) {
        list lParams = llParseString2List(sStr, [" "], []);

        if (llList2String(lParams, 0) != "shocker") {
            return;
        }
        ...
        if (sCommand == "help") {
            ...
            if (sAnim) {
                if (~llListFindList(g_lAnims, [sAnim])) {
                    g_sShockAnim = sAnim;
                } else {
                ...
                DialogSelectAnim(kID, iAuth);
            }
        } else if (sCommand == "sound") {
            string sSound = llStringTrim(sValue, STRING_TRIM);
            if (sSound) {
                if (sSound == g_sNoSound) {
                    llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "Punishment will be silently.", kID);
                ...
                DialogSelectSound(kID, iAuth);
            }
        } else {
            Shock((integer)sCommand);
        }
    }
        ...
        if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) {
            ...
            if (~iMenuIndex) {
                ...
                if (sMenu == "shocker") {
                    ...
                    } else if (sMsg == g_sSetAnim) {
                        DialogSelectAnim(kAv, iAuth);
                    } else if (sMsg == g_sSetSound) {
                        DialogSelectSound(kAv, iAuth);
                    } else if (sMsg == HELP) {
                        UserCommand(iAuth,"shocker help", kAv, TRUE);
                    } else {
                        integer iIndex = llListFindList(g_lButtons, [sMsg]);
                        if (iIndex != -1) {
                            UserCommand(iAuth, "shocker " + (string)llList2Integer(g_lTime, iIndex), kAv, TRUE);
                        }
                    }
                } else if (sMenu == "anim") {
                    if (sMsg != UPMENU) {
                        UserCommand(iAuth, "shocker animation " + sMsg, kAv, TRUE);
                    } else {
                        UserCommand(iAuth, "shocker", kAv, FALSE);
                    }
                } else if (sMenu == "sound") {
                    if (sMsg != UPMENU) {
                        UserCommand(iAuth, "shocker sound " + sMsg, kAv, TRUE);
                    } else {
                        UserCommand(iAuth, "shocker", kAv, FALSE);
                    }
                } else if (sMenu == "rmshocker") {
                    if (sMsg == "Yes") {
                        llMessageLinked(LINK_SAVE, LM_SETTING_DELETE, "shocker_anim", "");
