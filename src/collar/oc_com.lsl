...
integer g_bPublicListenChan = TRUE;
string g_sPrefix = ".";
...
// Globals for supporting touch requests
...
integer g_bNeedsPose = FALSE;  // Should the avatar be forced into a still pose for making touching easier
string g_sPoseAnim = "turn_180";

integer g_bTouchNotify = FALSE;  // For Touch Notify
integer g_bHighlander = TRUE; // There must be only one collar
list g_lCore5Scripts = [
  "LINK_AUTH", "oc_auth",
  "LINK_DIALOG", "oc_dialog",
  "LINK_RLV", "oc_rlvsys",
  "LINK_SAVE", "oc_settings",
  "LINK_ANIM", "oc_anim",
  "LINK_ANIM", "oc_couples"
];
...
ClearUser(key kRCPT, integer bNotify) {
  ...
  if (g_bNeedsPose && [] == g_lTouchRequests) {
    llStopAnimation(g_sPoseAnim);
  }
}
...
SendCommandFromLink(integer iLinkNumber, string sType, key kToucher) {
  ...
  for (; iTrig < iNTrigs; iTrig += g_iStrideLength) {
    if (llList2Key(g_lTouchRequests, iTrig + 1) == kToucher) {
      integer iTrigFlags = llList2Integer(g_lTouchRequests, iTrig + 2);
      if (((iTrigFlags & FLAG_TOUCHSTART) && sType == "touchstart") || ((iTrigFlags & FLAG_TOUCHEND) && sType == "touchend")) {
  ...
  if (sType == "touchstart") {
    ...
    if (g_bTouchNotify && kToucher != g_kWearer) {
      llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "\n\n" + NameURI((string)kToucher) + " touched your %DEVICETYPE%.\n", g_kWearer);
    }
  }
}
    ...
    if (sCommand == "prefix") {
      ...
      if (iAuth != CMD_OWNER) {
        ...
        if (sValue == "") { // Just let them know their current name
          sMessage = "\n\n" + NameURI((string)g_kWearer) + "'s current name is " + g_sWearerName;
      ...
      if (sValue=="") { // They left the param blank, report listener status
        ...
        if (g_bPublicListenChan) { // Save setting along with the state of thepublic listener (messy!)
...
SafeDelete(string sItem) {
  if (llGetInventoryType(sItem) == INVENTORY_NONE) {
    return;
  }

  if (llGetInventoryPermMask(sItem, MASK_OWNER) & PERM_COPY != PERM_COPY) {
    return;
  }
...
default {
  ...
  listen(integer iChan, string sSpeaker, key kID, string sMsg) {
    if (iChan == g_iHUDChan) {
      ...
      // It it is not a ping, it should be a command for use, to make sure it has to have the key in front of it
        ...
        if (sMsg == "OpenCollar?") {
          llRegionSayTo(g_kWearer, g_iInterfaceChannel, "OpenCollar=Yes");
        } else if (sMsg == "OpenCollar=Yes" && g_bHighlander) {
          llRegionSayTo(kID, g_iInterfaceChannel, "There can be only one!");
        } else if (sMsg == "There can be only one!" && llGetOwnerKey(kID) == g_kWearer && g_bHighlander) {
    ...
    if(llGetOwnerKey(kID) == g_kWearer) { // Also works for attachments
      string sSafeword = sMsg; // we'll have to shave pieces off as we go to test
      // safeword can be the safeword or safeword said in OOC chat "((SAFEWORD))"
      // and may include prefix
      if (llGetSubString(sSafeword, 0, 3) == "/me ") {
        sSafeword = llGetSubString(sSafeword, 4, -1);
      }
      // Allow for Firestorm style "(( SAFEWORD ))" by trimming.
      if (llGetSubString(sSafeword, 0, 1) == "((" && llGetSubString(sw, -2, -1) == "))") {
        sSafeword = llStringTrim(llGetSubString(sSafeword, 2, -3), STRING_TRIM);
      }
      if (llSubStringIndex(sw, g_sPrefix) == 0) {
        sSafeword = llGetSubString(sSafeword, llStringLength(g_sPrefix), -1);
      }
    ...
    if (iChan == g_iInterfaceChannel) {
      ...
      if (!llSubStringIndex(sMsg, g_sPrefix)) {
        sMsg = llGetSubString(sMsg, llStringLength(g_sPrefix), -1); // Strip our prefix from command
      } else if (!llSubStringIndex(sMsg, "/" + g_sPrefix)) {
        sMsg = llGetSubString(sMsg, llStringLength(g_sPrefix) + 1, -1); // Strip our prefix plus a / from command
      } else if (llGetSubString(sMsg, 0, 0) == "*") {
        sMsg = llGetSubString(sMsg, 1, -1); // Strip * (all collars wildcard) from command
      } else if ((llGetSubString(sMsg, 0, 0) == "#") && (kID != g_kWearer)) {
        sMsg = llGetSubString(sMsg, 1, -1); // Strip # (all collars but me) from command
      } else {
        return;
      }
  ...
  link_message(integer iSender, integer iNum, string sStr, key kID) {
    if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) {
      UserCommand(kID, iNum, sStr);
    } else if (iNum == LM_SETTING_RESPONSE) {
      list lParams = llParseString2List(sStr, ["="], []);
      string sToken = llList2String(lParams, 0);
      string sValue = llList2String(lParams, 1);

      if (sToken == g_sGlobalToken + "prefix" && sValue != "") g_sPrefix = sValue;
      else if (sToken == "leashpoint") g_iLeashPrim = (integer)sValue;
      else if (sToken == g_sGlobalToken + "DeviceName") g_sDeviceName = sValue;
      else if (sToken == g_sGlobalToken + "touchNotify") g_bTouchNotify = (integer)sValue; // for Touch Notify
      else if (sToken == "intern_Highlander") g_bHighlander = (integer)sValue;
      ...
      if (sStr != "LINK_REQUEST") {
        ...
        if (llGetListLength(g_lFoundCore5Scripts) >= 10) {
          llSetTimerEvent(0.1);
        }
      }
    ...
    } else if (iNum == AUTH_REPLY) { // Needed to be the same ID that send earlier pings or pongs
      llRegionSayTo(kID, g_iInterfaceChannel, sStr);
  ...
  timer() {
    ...
    do {
      iIndex = llListFindList(g_lFoundCore5Scripts, llList2List(g_lCore5Scripts, i, i));
      if (iIndex == -1) {
        if (llSubStringIndex(sMessage, llList2String(g_lCore5Scripts, i + 1)) == -1) {
          lTemp += llList2String(g_lCore5Scripts, i + 1);
        }
      } else {
        sMessage += "\n" + llList2String(g_lCore5Scripts, i + 1) + "\t(Link# " + llList2String(g_lFoundCore5Scripts, iIndex + 1) + ")";
      }
