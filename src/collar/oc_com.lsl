...
integer g_bHighlander = TRUE; // Only one OpenCollar should be worn
...
default {
    ...
    listen(integer iChan, string sSpeaker, key kID, string sMsg) {
        if (iChan == g_iHUDChan) {
            ...
            // If it is not a ping, it should be a command for use, to make sure it has to have the key in front of it
        ...
        if(llGetOwnerKey(kID) == g_kWearer) { // Also works for attachments
            string sSw = sMsg; // We'll have to shave pieces off as we go to test
            ...
            if (llGetSubString(sSw, 0, 1) == "((" && llGetSubString(sSw, -2, -1) == "))") {
                sSw = llStringTrim(llGetSubString(sSw, 2, -3), STRING_TRIM);
            }

            if (llSubStringIndex(llToLower(sSw), llToLower(g_sPrefix)) == 0) {
        ...
        if (iChan == g_iInterfaceChannel) {
            ...
            if (!llSubStringIndex(llToLower(sMsg), llToLower(g_sPrefix))) {
                sMsg = llGetSubString(sMsg, llStringLength(g_sPrefix), -1); // Strip our prefix from command
            } else if (!llSubStringIndex(llToLower(sMsg), "/" + llToLower(g_sPrefix))) {
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
        if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) {
            ...
            if (sToken == "leashpoint") g_iLeashPrim = (integer)sValue;
