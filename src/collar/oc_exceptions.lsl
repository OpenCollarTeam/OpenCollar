...
ExMenu(key kID, string sWho, integer iAuth) {
    ...
    if (~iIndex = llListFindList(g_lSettings, [sWho])) { // Replace deefault with custom
        iExSettings = llList2Integer(g_lSettings, iIndex + 1);
    }

    string sPrompt = "\nCurrent Settings for " + sWho + ": " + "\n";
    list lButtons;
    integer i;
    for (; i < llGetListLength(g_lPrettyCmds); i++) {
        // See if there's a setting for this in the settings list
        string sPretty = llList2String(g_lPrettyCmds, i);
        if (iExSettings & llList2Integer(g_lBinCmds, i)) {
            lButtons += [TURNOFF + " " + sPretty];
            sPrompt += "\n" + llList2String(g_lDescriptionsOn, i)+".";
        } else {
            lButtons += [TURNON + " " + sPretty];
            sPrompt += "\n" + llList2String(g_lDescriptionsOff, i)+".";
        }
    }
...
SetAllExs() {
    ...
    for (; n < iLength; ++n) {
        ...
        if (llListFindList(g_lSettings, [sTmpOwner]) == -1 && sTmpOwner != g_kWearer) {
            ...
            if ((string)lRLVCmd != "") {
                llOwnerSay("@" + llDumpList2String(lRLVCmd, ","));
            }
...
ClearEx() {
    if (g_iRLVOn) {
        llOwnerSay("@clear=" + llDumpList2String(g_lRLVcmds, ",clear="));
    }
}
...
UserCommand(integer iNum, string sStr, key kID) {
    ...
    integer iIndex = llGetListLength(lParts);
    if (iIndex < 1 || iIndex > 4 || llList2String(lParts, 0) != "ex") {
        return;
    }

    lParts = llDeleteSubList(lParts, 0, 0); // No longer need the "ex"
    iIndex = llGetListLength(lParts);
    string sCom = llList2String(lParts, 0);
    if (iIndex == 1) { // Handle requests 4 menus first
        if (sCom == "owner") {
            ExMenu(kID, "owner", iNum);
        } else if (sCom == "trusted") {
            ExMenu(kID, "trusted", iNum);
        }

        if (!llSubStringIndex(sCom, ":")) {
            jump UCDone; // Not done if we received a 1-who 1-exception case
        }
    }
    ...
    iIndex = llGetListLength(lParts) - 1;
    ...
    integer iL = 0;
    integer iC = 0;
    for (; iL < iIndex; iL += 2) { // Cycle through users
        ...
        string sWhoName;
        if ((key)sWho) {
            sWhoName = "secondlife:///app/agent/" + sWho + "/about";
        } else {
            sWhoName = sWho;
        }
        ...
        for (iC = 0; iC < llGetListLength(lCom); iC++) { // Cycle through strided entries
            ...
            if (sCom == "clear") {
                jump nextcom; // Do we want anything here this is for excpetions
            }
            ...
            if (sVal == "exempt" || sVal == "add") {
                sVal = "n"; // conversions
            } else if (sVal == "enforce" || sVal == "rem") {
                sVal = "y";
            }

            iRLV = llListFindList(g_lRLVcmds, [sCom]);
            if (iRLV == -1 && sCom != "defaults") {
                jump nextcom; // invalid request
            }

            iBin = llList2Integer(g_lBinCmds, iRLV);
            if (sWho == "owner") {
                if (sCom == "defaults") {
                    g_iOwnerDefault = OWNER_DEFAULT;
                } else if (sVal == "n") {
                    g_iOwnerDefault = g_iOwnerDefault | iBin;
                } else if (sVal == "y") {
                    g_iOwnerDefault = g_iOwnerDefault & ~iBin;
                }
                ...
                if (sCom == "defaults") {
                    g_iTrustedDefault = TRUSTED_DEFAULT;
                } else if (sVal == "n") {
                    g_iTrustedDefault = g_iTrustedDefault | iBin;
                } else if (sVal == "y") {
                    g_iTrustedDefault = g_iTrustedDefault & ~iBin;
                }
            ...
            if (sCom == "defaults") {
                if (~iNames) {
                    g_lSettings = llDeleteSubList(g_lSettings, iNames, iNames + 1);
                }

                /*
                if (~iNames = llListFindList(g_lNames, [sWho])) {
                    g_lNames = llDeleteSubList(g_lNames, iNames, iNames + 1);
                }
                */
            ...
            if (~iNames) {
                iSet = llList2Integer(g_lSettings, iNames + 1);
            } else if (~llListFindList(g_lOwners, [sWho])) {
                iSet = g_iOwnerDefault;
            } else if (~llListFindList(g_lSecOwners, [sWho])) {
                iSet = g_iTrustedDefault;
            } else {
                iSet = 0;
            }
            ...
            if (sVal == "n") {
                ...
                jump nextcom; // invalid setting param
            }

            if (~iNames) {
                g_lSettings = llListReplaceList(g_lSettings, [iSet], iNames + 1, iNames + 1);
            } else {
                g_lSettings += [sWho, iSet];
            }
