...
list ATTACH_POINT_ROTATIONS = [
  ATTACH_BACK, <-90.0, 0.0, 0.0>,
  ATTACH_CHEST, <90.0, 0.0, 0.0>
];
...
ShowHideTitle() {
  if (g_iTextPrim > 0) {
    ...
    llSetLinkPrimitiveParamsFast(g_iTextPrim, [
      PRIM_TEXT, g_sText, g_vColor, (float)g_bOn,
      PRIM_SIZE, g_vPrimScale,
      PRIM_SLICE, <0.490, 0.51, 0.0>
    ]);
  }
  ...
  if (g_iPartPrim > 0) {
    if (g_sParticle == "") {
      ...
      integer iIndex = llListFindList(ATTACH_POINT_ROTATIONS, [llGetAttached()]);
      if (~iIndex) {
        rStart = llEuler2Rot(llList2Vector(ATTACH_POINT_ROTATIONS, iIndex + 1) * DEG_TO_RAD);
      }
      vector vPos = (g_vPartOffset + <0.0, 0.0, g_vPrimScale.z>) / (rStart * llGetLocalRot());
...
AssembleTextures() {
  g_lImages = g_lStdImages;
  integer iCount = llGetInventoryNumber(INVENTORY_TEXTURE);
  integer i;
  for (; i < iCount; i++) {
    string sName = llGetInventoryName(INVENTORY_TEXTURE, i);
    key kTextureId = llGetInventoryKey(sName);
    g_lImages += [sName, kTextureId];
  }
}

ImageMenu(key kAv, integer iAuth) {
  Dialog(kAv, "Hang an image overhead", ["OFF", "BIGGER", "SMALLER", "CUSTOM"] + llList2ListStrided(g_lImages, 0, -1, 2), [UPMENU], 0, iAuth, "image");
}

string FindImage(string sName) {
  integer iIndex = llListFindList(g_lImages, [sName]);
  if (iIndex < 0 || iIndex % 2 != 0) {
    return sName;
  }
  return llList2String(g_lImages, iIndex + 1);
}
...
TitlerMenu(key kAv, integer iAuth) {
  ...
  if (g_iTextPrim == -1) {
    ...
    Dialog(kAv, sPrompt, [SET, UP, DN, sOnOff, "Color", "Image"], [UPMENU], 0, iAuth, "main");
  }
}
...
UserCommand(integer iAuth, string sStr, key kAv) {
  ...
  if (sLowerStr == "menu titler" || sLowerStr == "titler") {
    ...
    if (g_bOn && iAuth > g_iLastRank) { // Only change text if commander has same or greater auth
      ...
      g_iLastRank = iAuth;
      g_bOn = TRUE;
      llMessageLinked(LINK_SAVE, LM_SETTING_SAVE, g_sSettingToken + "on=" + (string)g_bOn, "");
      llMessageLinked(LINK_SAVE, LM_SETTING_SAVE, g_sSettingToken + "auth=" + (string)g_iLastRank, ""); // Save lastrank to DB
    } else if (sAction == "off" && bIsCommand) {
      g_iLastRank = CMD_EVERYONE;
      g_bOn = FALSE;
      llMessageLinked(LINK_SAVE, LM_SETTING_DELETE, g_sSettingToken + "on", "");
      llMessageLinked(LINK_SAVE, LM_SETTING_DELETE, g_sSettingToken + "auth", ""); // Delete lastrank from DB
    ...
    } else if (sAction ==  "down" && bIsCommand) {
      g_vPrimScale.z -= 0.05;
...
default {
  state_entry() {
    g_iTextPrim = -1;
    integer iLinkNumber = llGetNumberOfPrims() + 1;
    while (iLinkNumber-- > 2) {
      string sDesc = llList2String(llGetLinkPrimitiveParams(iLinkNumber, [PRIM_DESC]), 0);
      if (llSubStringIndex(sDesc, g_sPrimDesc) == 0) {
        g_iTextPrim = iLinkNumber;
        llSetLinkPrimitiveParamsFast(g_iTextPrim, [
        ...
        llLinkParticleSystem(iLinkNumber, []);
      } else {
        if (llSubStringIndex(sDesc, g_sPartDesc) == 0) {
          g_iPartPrim = iLinkNumber;
          llSetLinkPrimitiveParamsFast(g_iPartPrim, [
            PRIM_TYPE, PRIM_TYPE_CYLINDER, 0, <0.0, 1.0, 0.0>, 0.95, ZERO_VECTOR, <1.0, 1.0, 0.0>, ZERO_VECTOR,
            PRIM_TEXTURE, ALL_SIDES, TEXTURE_TRANSPARENT, <1.0, 1.0, 0.0>, ZERO_VECTOR, 0.0,
            PRIM_SIZE, <0.01, 0.01, 0.01>,
            PRIM_SLICE, <0.49, 0.51, 0.0>,
            PRIM_DESC, g_sPartDesc + "~notexture~nocolor~nohide~noshiny~noglow"
          ]);
        }
        llSetLinkPrimitiveParamsFast(iLinkNumber, [
          PRIM_TEXT, "", <0.0, 0.0, 0.0>, 0
        ]);
        llLinkParticleSystem(iLinkNumber, []);
      }
    }
  ...
  link_message(integer iSender, integer iNum, string sStr, key kID) {
    if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) {
      ...
      if (sGroup == g_sSettingToken) {
        ...
        else if (sToken == "on") g_bOn = (integer)sValue;
        ...
        else if (sToken == "particle") g_sParticle = sValue;
        else if (sToken == "particlesize") g_vPartSize = (vector)sValue;
      ...
      if (~iMenuIndex) {
        ...
        if (sMenuType == "main") {
          ...
          if (sMessage == "") {
            ...
            UserCommand(iAuth, "image " + llToLower(sMessage), kAv);
          }
          ...
          if (sMessage != " ") {
            UserCommand(iAuth, "title " + sMessage, kAv);
          }
