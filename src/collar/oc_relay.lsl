...
list g_lQueue = [];
integer QSTRIDES = 3;
integer g_bAuthPending = FALSE;
integer g_bRecentSafeword;
...
//settings
integer g_bMinBaseMode = FALSE;
integer g_bMinHelplessModeMode = FALSE;
integer g_bMinLandMode = FALSE;
integer g_bMinLiteMode = FALSE;
integer g_iBaseMode = 2;
integer g_iHelpless = 0;
integer g_iLandMode = 0;
integer g_iLiteMode = 0;

integer g_bSmartStrip = TRUE; // Convert @remoutfit to @detachallthis.
...
key SanitizeKey(string sUUID) {
    if ((key)sUUID) {
        return llToLower(sUUID);
    }
    return NULL_KEY;
}

RelayNotify(key kID, string sMessage, integer bNofityWearer) {
...
SaveTrustObj() {
    if (llGetListLength(g_lTrustObj) > 0) {
        ...
        llMessageLinked(LINK_SAVE, LM_SETTING_DELETE, g_sSettingsToken + "trustobj", "");
    }
}
...
SaveBlockObj() {
    if (llGetListLength(g_lBlockObj) > 0) {
        ...
        llMessageLinked(LINK_SAVE, LM_SETTING_DELETE, g_sSettingsToken + "blockobj", "");
    }
}
*/
...
SaveTrustAv() {
    if (llGetListLength(g_lTrustAv) > 0) {
        ...
        llMessageLinked(LINK_SAVE, LM_SETTING_DELETE, g_sSettingsToken + "trustav", "");
    }
}
...
UpdateMode(integer iMode) {
    ...
    g_bMinBaseMode = (iMode >> 5) & 3;
    g_bMinHelplessModeMode = (iMode >> 7) & 1;
    g_bMinLandMode = (iMode >> 8) & 1;
    g_bMinLiteMode = (iMode >> 9) & 1;
    g_bSmartStrip = (iMode >> 10) & 1;
}
...
integer Auth(key kObject, key kUser) {
    integer iAuth = 1;
    key kOwner = llGetOwnerKey(kObject);
    // Object auth
    integer iSourceIndex = llListFindList(g_lSources, [kObject]);
    if (~iSourceIndex) {
    } else if (~llListFindList(g_lTempBlockObj + g_lBlockObj, [kObject])) {
        return -1;
    } else if (~llListFindList(g_lBlockAv, [(string)kOwner])) {
        return -1;
    } else if (~llListFindList(g_lBlock, [(string)kOwner])) {
        return -1;
    } else if (g_iBaseMode == 3) {
    } else if (g_iLandMode && llGetOwnerKey(kObject) == llGetLandOwnerAt(llGetPos())) {
    } else if (~llListFindList(g_lTempTrustObj + g_lTrustObj, [kObject])) {
    ...
    /*
        ...
        return -1; // We should not block playful in trusted mode
    */
    } else {
        iAuth = 0;
    }
    ...
    if (kUser) {
        ...
        //else
        if (~llListFindList(g_lBlockAv + g_lTempBlockUser, [kUser])) {
            return -1;
        } else if (~llListFindList(g_lBlock, [(string)kUser])) {
            return -1;
        } else if (g_iBaseMode == 3) {
        } else if (~llListFindList(g_lTrustAv + g_lTempTrustUser, [kUser])) {
        } else if (~llListFindList(g_lOwner + g_lTrust + g_lTempOwner, [(string)kUser])) {
        /*
        } else if (g_iBaseMode == 1) {
            return -1;
        */
...
Dequeue() {
    ...
    list lButtons = [
        "Yes", "Trust Owner", "Trust Object",
        "No", "Block Owner", "Block Object"
    ];
...
string HandleCommand(string sIdent, key kID, string sCom, integer bAuthed) {
    list lCommands = llParseString2List(sCom, ["|"], []);
    sCom = llList2String(lCommands, 0);
    integer bGotWho = FALSE; // Has the user been specified up to now?
    key kWho;
    integer i;
    for (i; i < (lCommands != []); ++i) {
        ...
        if (sCom == "!release" || sCom == "@clear") {
            ...
            /*
            if (bIsWho) {
                return llList2String(lCommands, 0) + "|" + llDumpList2String(llList2List(lCommands, i, -1), "|");
            } else {
                return llDumpList2String(llList2List(lCommands, i, -1), "|");
            }
            */
            ...
            if ((integer)sVal) {
                llMessageLinked(LINK_RLV,RLV_CMD, llGetSubString(sCom, 1, -1), kID); // Now with RLV 1.23, negative channels can also be used
            } else {
                sAck = "ko";
            }
            ...
            if (bGotWho) {
                return "!x-who/" + (string)kWho + "|" + llDumpList2String(llList2List(lCommands, i, -1), "|");
            } else {
                return llDumpList2String(llList2List(lCommands, i, -1), "|");
            }
        } else if ((lSubArgs != []) == 2) {
            ...
            if (g_bSmartStrip && llList2String(lTemp, 0) == "remoutfit" && sVal == "force") {
        ...
        if (sAck) {
            SendRLVRelayMessage(sIdent, kID, sCom, sAck);
        }
    }
...
SendRLVRelayMessage(string sIdent, key kID, string sCom, string sAck) {
...
SafeWord() {
    if (!g_iHelpless) {
        ...
        integer i;
        for (; i < (g_lSources != []); ++i) {
            SendRLVRelayMessage("release", llList2Key(g_lSources, i), "!release", "ok");
        }
        g_lSources = [];
        g_bRecentSafeword = TRUE;
        RefreshRLVListener();
        llSetTimerEvent(30.);
    } else {
...
Menu(key kID, integer iAuth) {
    ...
    lButtons += [
        ...
        Checkbox(g_bSmartStrip, "Smart"),
        Checkbox(g_iLandMode, "Land"),

        "Pending", "Sources", "Access Lists",
    ...
    if (g_lSources != []) {
        ...
        if (g_lSources == [1]) { // Note: only list LENGTH is compared here
            sPrompt += ".";
        } else {
            sPrompt += "s.";
        }
...
RemoveList(string sMsg, integer iAuth, string sListType) {
    ...
    if (sListType == "Block Avatars") {
        if (sMsg == ALL) {
            ...
            iIndex = llListFindList(g_lBlockAv, [sMsg]);
            if (~iIndex) {
                g_lBlockAv = llDeleteSubList(g_lBlockAv, iIndex, iIndex);
            }
        }
        ...
        if (sMsg == ALL) {
            ...
            iIndex = llListFindList(g_lBlockObj, [sMsg]);
            if (~iIndex) {
                g_lBlockObj = llDeleteSubList(g_lBlockObj, iIndex - 1, iIndex);
            }
        }

        //SaveBlockObj();
    } else if (iAuth == CMD_WEARER && g_bMinBaseMode > 0) {
        RelayNotify(g_kWearer, "Access denied!", 0);
        return;
        ...
        if (sMsg == ALL) {
            ...
            iIndex = llListFindList(g_lTrustObj, [sMsg]);
            if (~iIndex) {
                g_lTrustObj = llDeleteSubList(g_lTrustObj, iIndex - 1, iIndex);
            }
        }
        ...
        if (sMsg == ALL) {
            ...
            iIndex = llListFindList(g_lTrustAv, [sMsg]);
            if (~iIndex) {
                g_lTrustAv = llDeleteSubList(g_lTrustAv, iIndex, iIndex);
            }
        }
...
RefreshRLVListener() {
    ...
    if (g_bRLV && g_iBaseMode && !g_bRecentSafeword) {
...
CleanQueue() {
    ...
    while (i < (g_lQueue != []) / QSTRIDES) { // GetQLength()
        ...
        key kUser = NULL_KEY;
        integer bGotWho = llGetSubString(sCommand, 0, 6)=="!x-who/";
        ...
        if (~llListFindList(lOnHold, [kObj])) {
            ...
            integer j;
            for (; j < (lCommands != []); ++j) {
                SendRLVRelayMessage(sIdent, kObj, llList2String(lCommands, j), "ko");
            }
        } else {
            ++i;
            lOnHold += kObj;
...
UserCommand(integer iNum, string sStr, key kID) {
    ...
    if (!g_bRLV) {
        ...
        RelayNotify(kID, "Access denied!", 0);
    } else if ((sStr = llGetSubString(sStr, 6, -1)) == "safeword") { // cut "relay " off sStr
        SafeWord();
        ...
        if (sChangeType == "helpless") {
            if (g_lSources != []) {
                ...
                if (iNum == CMD_OWNER) {
                    g_bMinHelplessModeMode = FALSE;
                }
                if (g_bMinHelplessModeMode == TRUE) {
                    iWSuccess = 1;
                } else {
                    if (iNum == CMD_OWNER) {
                        g_bMinHelplessModeMode = FALSE;
            ...
            if (sChangeValue == "off") {
                if (iNum == CMD_OWNER) {
                    g_bMinLandMode = FALSE;
                }
                if (g_bMinLandMode == TRUE) {
                    iWSuccess = 1;
                } else {
            ...
            if (sChangeValue == "off") {
                if (iNum == CMD_OWNER) {
                    g_bMinLiteMode = FALSE;
                }
                if (g_bMinLiteMode == TRUE) {
                    iWSuccess = 1;
                } else {
            ...
            if (iNum == CMD_OWNER) {
                g_bMinBaseMode = iModeType;
            }
            if (~iModeType) {
                if (iModeType >= g_bMinBaseMode) {
                    if (iModeType) {
                        sText = "/me is set to " + llList2String(lModes, iModeType) + " mode.";
                    } else {
                        sText = "/me is offline.";
                    }
        ...
        if (!iWSuccess) {
            RelayNotify(kID, sText, 1);
        } else if (iWSuccess == 1) {
            RelayNotify(kID, "Access denied!", 0);
        } else if (iWSuccess == 2) {
            RelayNotify(kID, "/me is currently in use by one or more sources.\n\nHelplessness can't be toggled at this moment.\n", 1);
        /*
        } else if (iWSuccess == 3) {
            RelayNotify(kID, "Invalid command, please read the manual.", 0);
        */
        }
...
default {
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
        if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) {
            ...
            integer iIndex = llListFindList(g_lSources,[kID]);
            if (~iIndex) {
                g_lSources = llDeleteSubList(g_lSources, iIndex, iIndex);
            }
            ...
            if (~iMenuIndex) {
                ...
                if (sMenu == "Menu~Main") {
                    ...
                    if (sMsg == "Yes") {
                        ...
                        if (kUser) {
                            g_lTempTrustUser += (string)kUser;
                        }
                    } else if (sMsg == "No") {
                        g_lTempBlockObj += kCurID;
                        if (kUser) {
                            g_lTempBlockUser += (string)kUser;
                        }
                    ...
                    if (sMsg == "Yes") {
                        ...
                        RelayNotify(kAv, "/me remains installed.", 0);
                    }
                }
            }
    ...
    listen(integer iChannel, string sWho, key kID, string sMsg) {
        ...
        // send the ping to rlvmain to manage restrictions of this old source
        } else if (llStringLength(sMsg) > 700) { // Too long command, will make the relay crash in ask mode
            sMsg = "";
            ...
            llSetTimerEvent(30.);
            return;
        } else { // Un other cases we analyze the command here
        */
        ...
        if (iAuth == -1) {
            ...
            if (llGetMemoryLimit() - llGetUsedMemory() < 3927) { // Keeps margin for this event + next arriving chat message
                ...
                if (llGetSubString(sMsg, 0, 6) == "!x-who/") {
                    kOldUser = SanitizeKey(llGetSubString(llList2String(g_lQueue, 2), 7, 42));
                }
                */
    ...
    timer() {
        if (g_bRecentSafeword) {
            g_bRecentSafeword = FALSE;
            RefreshRLVListener();
        }
        ...
        for (; i < (g_lSources != []); ++i) {
            ...
            if (vObjPos == <0.0, 0.0, 0.0> || llVecDist(vObjPos, vMyPos) > 100) { // 100: max shout distance
                llMessageLinked(LINK_RLV, RLV_CMD, "clear", kID);
            }
        }
        ...
        g_lTempTrustObj = [];
        if (g_lSources == []) {
            // Don't clear already authorized users before done with current session
            g_lTempBlockUser = [];
            g_lTempTrustUser = [];
        }
    }
