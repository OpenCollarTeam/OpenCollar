...
key g_kGroup = "";
integer g_bGroupEnabled = FALSE;

string g_sParentMenu = "Main";
string g_sSubMenu = "Access";
integer g_bRunawayDisabled = FALSE;
...
integer g_bOpenAccess; // 0: disabled, 1: openaccess
integer g_bLimitRange = TRUE; // 0: disabled, 1: limited
integer g_bOwnSelf; // self-owned wearers
string g_sFlavor = "OwnSelf";

list g_lMenuIDs;
integer g_iMenuStride = 3;
key g_kConfirmOwnSelfOffDialogID;
integer g_bGrantRemoval;
//key REQUEST_KEY;
integer g_bFirstRun;
...
Dialog(string sID, string sPrompt, list lChoices, list lUtilityButtons, integer iPage, integer iAuth, string sName, integer bSensor) {
...
AuthMenu(key kAv, integer iAuth) {
    ...
    list lButtons = [
        ...
        Checkbox(g_bGroupEnabled, "Group"),
        Checkbox(g_bOpenAccess, "Public"),
        Checkbox(g_bOwnSelf, g_sFlavor),
...
RemPersonMenu(key kID, string sToken, integer iAuth) {
    ...
    if (lPeople) {
        ...
        integer i;
        for(; i < iNum; i++) {
            string sName = llList2String(lPeople, i);
            if (sName) {
                lButtons += sName;
            }
        }
...
RemovePerson(string sPersonID, string sToken, key kCmdr, integer bPromoted) {
    list lPeople;
    ...
    integer bFound;
    if (lPeople) {
        integer iIndex = llListFindList(lPeople, [sPersonID]);
        if (~iIndex) {
            if (sToken == "owner" && sPersonID == g_sWearerID && !g_bGrantRemoval) {
                string sMsg;
                if ((key)g_sWearerID == kCmdr) {
                    sMsg = "Are you sure you no longer wish to own yourself?";
                } else {
                    sMsg = NameURI(kCmdr) + " wants to remove you as owner. Do you agree?";
                }
                g_kConfirmOwnSelfOffDialogID = llGenerateKey();
                llMessageLinked(LINK_DIALOG, DIALOG, g_sWearerID + "|" + sMsg + "|0|Yes`No|Cancel|", g_kConfirmOwnSelfOffDialogID);
            } else {
                //OwnSelfOff(kCmdr);
                lPeople = llDeleteSubList(lPeople, iIndex, iIndex);
                if (!bPromoted) {
                    llMessageLinked(LINK_DIALOG, NOTIFY, "0" + NameURI(sPersonID) + " removed from " + sToken + " list.", kCmdr);
                }
                bFound = TRUE;
            }
        } else if (llToLower(sPersonID) == "remove all" && !g_bGrantRemoval) {
            if (sToken == "owner" && ~llListFindList(lPeople, [g_sWearerID])) {
                ...
                g_kConfirmOwnSelfOffDialogID = llGenerateKey();
                llMessageLinked(LINK_DIALOG, DIALOG, (string)llGetOwner() + "|" + sMsg + "|0|Yes`No|Cancel|", g_kConfirmOwnSelfOffDialogID);
            } else {
...
AddUniquePerson(string sPersonID, string sToken, key kID) {
    ...
    if (~llListFindList(g_lTempOwner, [(string)kID]) && ! ~llListFindList(g_lOwner, [(string)kID]) && sToken != "tempowner") {
        ...
        // Put a cap on how many people we'll remember, to avoid running out of memory.
        integer iPeopleStored = llGetListLength(g_lOwner) +
            llGetListLength(g_lTrust) +
            llGetListLength(g_lTempOwner) +
            llGetListLength(g_lBlock);

        if (iPeopleStored >= g_iPeopleCap) {
        ...
        if (!~llListFindList(lPeople, [sPersonID])) { // Owner is not already in list.  add him/her
            lPeople += sPersonID;
            if (sPersonID == g_sWearerID && sToken == "owner") {
                g_bOwnSelf = TRUE;
            }
        } else {
        ...
        if (sPersonID != g_sWearerID) {
            llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "Building relationship...", g_sWearerID);
        }
        ...
        if (sToken == "owner") {
            ...
            if (~llListFindList(g_lBlock,[sPersonID])) RemovePerson(sPersonID, "block", kID, TRUE);
            if (sPersonID != g_sWearerID) {
                llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "Looks like " + NameURI(sPersonID) + " is someone you can trust!", g_sWearerID);
        ...
        if (sToken=="owner") {
            ...
            if (llGetListLength(g_lOwner) > 1 || sPersonID != g_sWearerID) {
                SayOwners();
            }
        }
        else if (sToken=="trust") g_lTrust = lPeople;
...
SayOwners() { // Give a "you are owned by" message, nicely formatted.
    ...
    if (iCount) {
        list lTemp = g_lOwner;
        integer iIndex = llListFindList(lTemp, [g_sWearerID]);
        // If wearer is also owner, move the key to the end of the list.
        if (~iIndex) {
            lTemp = llDeleteSubList(lTemp, iIndex, iIndex) + [g_sWearerID];
        }

        string sMsg = "You belong to ";
        string sTemp;
        if (iCount == 1) {
            sTemp = llList2String(lTemp, 0);
            ...
            iIndex = 0;
            do {
                sMsg += NameURI(llList2String(lTemp, iIndex)) + ", ";
                iIndex++;
            } while (iIndex < iCount - 1);

            sTemp = llList2String(lTemp, iIndex);
            if (sTemp == g_sWearerID) {
                sMsg += "and yourself.";
            } else {
                sMsg += "and " + NameURI(sTemp) + ".";
            }
        }
...
integer InRange(key kID) {
    if (g_bLimitRange) {
        // If the distance between my position and their position  > 20
...
integer Auth(string sObjID) {
    ...
    if (~llListFindList(g_lOwner + g_lTempOwner, [sID])) {
        ...
        iNum = CMD_WEARER;
    } else if (g_bOpenAccess) {
        if (InRange((key)sID)) {
            iNum = CMD_GROUP;
        } else {
            iNum = CMD_EVERYONE;
        }
    } else if (g_bGroupEnabled && (string)llGetObjectDetails((key)sObjID, [OBJECT_GROUP]) == (string)g_kGroup && (key)sID != g_sWearerID) {
        // Meaning that the command came from an object set to our control group, and is not owned by the wearer
        iNum = CMD_GROUP;
    } else if (llSameGroup(sID) && g_bGroupEnabled && sID != g_sWearerID) {
        if (InRange((key)sID)) {
            iNum = CMD_GROUP;
        } else {
            iNum = CMD_EVERYONE;
...
UserCommand(integer iNum, string sStr, key kID, integer bRemenu) { // here iNum: auth value, sStr: user command, kID: avatar id
    ...
    if (sStr == "menu " + g_sSubMenu) {
        ...
        if (iNum == CMD_OWNER || kID == g_sWearerID) {
            ...
            if (sOutput) {
                ...
                llMessageLinked(LINK_DIALOG,NOTIFY,"0" + "Owners: none", kID);
            }
            ...
            if (sOutput) {
                llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"Temporary Owner: " + sOutput, kID);
            }
            ...
            if (g_kGroup) {
                llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "Group: secondlife:///app/group/" + (string)g_kGroup + "/about", kID);
            }
        ...
        if (iNum == CMD_OWNER && !~llListFindList(g_lTempOwner, [(string)kID])) {
            if (sAction == "on") {
                ...
                if (g_sWearerID == kID) {
                    ...
                    sMsg = llGetDisplayName(kID) + " wants to remove you as owner. Do you agree?";
                }
                llMessageLinked(LINK_DIALOG, DIALOG, (string)llGetOwner() + "|" + sMsg + "|0|Yes`No|Cancel|", g_kConfirmOwnSelfOffDialogID);
                //g_bOwnSelf = FALSE;
                bRemenu = FALSE;
        ...
        AuthMenu(kID, iNum);
    } else if (sCommand == "owner" && bRemenu == FALSE) { // Request for access menu from chat
        AuthMenu(kID, iNum);
    } else if (sCommand == "add") { // Add a person to a list
        if (!~llListFindList(["owner", "trust", "block"], [sAction])) {
        ...
        if (iNum != CMD_OWNER && !(sAction == "trust" && kID == g_sWearerID)) {
            ...
            if (bRemenu) {
                RemPersonMenu(kID, sAction, Auth(kID));
            }
        ...
        if (iNum == CMD_OWNER){
            if (sAction == "on") {
                ...
                if ((key)(llList2String(lParams, -1))) {
                    ...
                    g_kGroup = (key)llList2String(llGetObjectDetails(llGetKey(), [OBJECT_GROUP]), 0); // Record current group key
                }

                if (g_kGroup != "") {
                    llMessageLinked(LINK_SAVE, LM_SETTING_SAVE, g_sSettingToken + "group=" + (string)g_kGroup, "");
                    g_bGroupEnabled = TRUE;
        ...
        if (iNum == CMD_OWNER) {
            if (sAction == "on") {
                g_bOpenAccess = TRUE;
                llMessageLinked(LINK_SAVE, LM_SETTING_SAVE, g_sSettingToken + "public=" + (string)g_bOpenAccess, "");
        ...
        if (iNum == CMD_OWNER){
            if (sAction == "on") {
                ...
                llMessageLinked(LINK_SAVE, LM_SETTING_SAVE, g_sSettingToken + "limitrange=" + (string)g_bLimitRange, "");
        ...
        if (kID == g_sWearerID){  // Wearer called for menu
            ...
            llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "This feature is only for the wearer of the %DEVICETYPE%.", kID);
        }
        ...
        if (iNum == CMD_ZERO) { // Authenticate messages on CMD_ZERO
            ...
            if (llGetSubString(sToken, 0, i) == g_sSettingToken) {
                ...
                if (sToken == "owner") {
                    ...
                    if (~llSubStringIndex(sValue, g_sWearerID)) {
                        g_bOwnSelf = TRUE;
                    } else {
                        g_bOwnSelf = FALSE;
                    }
                ...
                else if (sToken == "public") g_bOpenAccess = (integer)sValue;
                else if (sToken == "limitrange") g_bLimitRange = (integer)sValue;
                else if (sToken == "norun") g_bRunawayDisabled = (integer)sValue;
                ...
                if (llGetListLength(g_lOwner) && g_bFirstRun) {
                    SayOwners();
                    g_bFirstRun = FALSE;
                }
            }
        } else if (iNum == AUTH_REQUEST) { //The reply is: "AuthReply|UUID|iAuth", we reroute this to com to have the same prim ID
            llSetLinkPrimitiveParamsFast(LINK_THIS, [
                PRIM_FULLBRIGHT, ALL_SIDES, TRUE,
                PRIM_BUMP_SHINY, ALL_SIDES, PRIM_SHINY_NONE, PRIM_BUMP_NONE,
                PRIM_GLOW, ALL_SIDES, 0.4
            ...
            if (~iMenuIndex) {
                ...
                if (sMenu == "Auth") {
                    if (sMessage == UPMENU) {
                        ...
                        integer iButtonIndex = llListFindList(lTranslation, [sMessage]);
                        if (~iButtonIndex) {
                            sMessage = llList2String(lTranslation, iButtonIndex + 1);
                        }
                    ...
                    if (sMessage == "Yes") {
                        ...
                        llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "Runaway aborted.", kAv);
                    } else if (sMessage == UPMENU) {
                        AuthMenu(kAv, iAuth);
                    }
                } if (llSubStringIndex(sMenu,"AddAvi") == 0) {
                    if ((key)sMessage) {
                        AddUniquePerson(sMessage, llGetSubString(sMenu, 6, -1), kAv); // Should be safe to use key2name here, as we added from sensor dialog
                    } else if (sMessage == "BACK") {
                        AuthMenu(kAv, iAuth);
                    }
                }
            } else if ( kID == g_kConfirmOwnSelfOffDialogID) {
                list MenuParams = llParseString2List(sStr, ["|"], []);
                if (llList2String(MenuParams, 1) == "Yes") {
                    // truly disable ownself now
                    g_bOwnSelf = FALSE;
                    g_bGrantRemoval = TRUE;
