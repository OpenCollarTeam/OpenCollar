...
Dialog(key kRCPT, string sPrompt, list lButtons, list lUtilityButtons, integer iPage, integer iAuth, string sMenuID) {
    ...
    if (sMenuID == "sensor") {
        if (lButtons) {
            sSearch = "`" + llList2String(lButtons, 0) + "`1";
        }
...
RefreshTimer() {
    ...
    if (g_iAnimTimeout <= iTimeNow && g_iAnimTimeout > 0) {
        //Debug("Anim timeout=" + (string)g_iAnimTimeout + "\ntime now=" + (string)iTimeNow);
        g_iAnimTimeout = 0;
        StopAnims();
    } else if (g_iPermissionTimeout <= iTimeNow && g_iPermissionTimeout > 0) {
    ...
    integer iNextTimeout = g_iAnimTimeout;
    if (g_iPermissionTimeout < g_iAnimTimeout && g_iPermissionTimeout > 0) {
        iNextTimeout = g_iPermissionTimeout;
    }
    llSetTimerEvent(iNextTimeout - iTimeNow);
}
...
CoupleAnimMenu(key kID, integer iAuth) {
    ...
    if (g_fTimeOut == 0) {
        ...
        sPrompt += "for " + (string)llCeil(g_fTimeOut) + " seconds.";
    }
...
string StrReplace(string sSrc, string sFrom, string sTo) {
    ...
    if (~iLength)  {
        ...
        if (iToPos) {
            iPos -= iToPos;
            sSrc = llInsertString(llDeleteSubString(sSrc, iPos, iPos + iLength), iPos, sTo);
            iPos += iToLen;
            sBuffer = llGetSubString(sSrc, (-~(iPos)), 0x8000);
            //buffer = llGetSubString(sSrc = llInsertString(llDeleteSubString(sSrc, iPos -= iToPos, iPos + iLength), iPos, sTo), (-~(iPos += iToLen)), 0x8000);
            jump loop;
        }
    }
    return sSrc;
...
StopAnims() {
    if (llGetInventoryType(g_sSubAnim) == INVENTORY_ANIMATION) {
        llMessageLinked(LINK_THIS, ANIM_STOP, g_sSubAnim, "");
    }
...
MoveToPartner() {
    list lPartnerDetails = llGetObjectDetails(g_kPartner, [OBJECT_POS, OBJECT_ROT]);
    vector vPartnerPos = llList2Vector(lPartnerDetails, 0);
    rotation rPartnerRot = llList2Rot(lPartnerDetails, 1);
    vector vPartnerEuler = llRot2Euler(rPartnerRot);

    // Turn to face the partner
    llMessageLinked(LINK_RLV, RLV_CMD, "setrot:" + (string)(-PI_BY_TWO - vPartnerEuler.z) + "=force", NULL_KEY);

    g_iTargetID = llTarget(vPartnerPos, g_fWalkingDistance);
    llMoveToTarget(vPartnerPos, g_fWalkingTau);
}
...
default {
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
        ...
        if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) {
            ...
            integer iTmpiIndex = llListFindList(g_lAnimCmds, [sCommand]);
            if (iTmpiIndex != -1) { // If the couple anim exists
                g_iCmdIndex = iTmpiIndex;
                //Debug(sCommand);
            ...
            } else if (llToLower(sStr) == "stop couples") {
                StopAnims();
                ...
                if (sValue == "off") {
                    g_bVerbose = FALSE;
                    llMessageLinked(LINK_SAVE, LM_SETTING_SAVE, g_sSettingToken + "verbose=" + (string)g_bVerbose, "");
                } else if (sValue == "on") {
                    g_bVerbose = TRUE;
            ...
            if (sToken == g_sSettingToken + "timeout") g_fTimeOut = (float)sValue;
            else if (sToken == g_sSettingToken + "verbose") g_bVerbose = (integer)sValue;
    ...
    at_target(integer iTargetNum, vector vTargetPos, vector vOurPos) {
        llTargetRemove(iTargetNum);
        llStopMoveToTarget();

        float fOffset = 10.0;
        if (g_iCmdIndex != -1) {
            fOffset = (float)llList2String(g_lAnimSettings, g_iCmdIndex * 4 + 2);
        }

        list lPartnerDetails = llGetObjectDetails(g_kPartner, [OBJECT_POS, OBJECT_ROT]);
        vector vPartnerPos = llList2Vector(lPartnerDetails, 0);
        rotation rPartnerRot = llList2Rot(lPartnerDetails, 1);
        vector vMyPos = llList2Vector(llGetObjectDetails(llGetOwner(), [OBJECT_POS]), 0);

        vector vTarget = vPartnerPos + (<1.0, 0.0, 0.0> * rPartnerRot * fOffset); // Target is <offset> meters in front of the partner
        vTarget.z = vMyPos.z; // ignore height differences
        llMoveToTarget(vTarget, g_fAlignTau);
        ...
        if (g_fTimeOut > 0.0) {
            ...
            if (g_sSubAnim == "~good" && g_fTimeOut > 20.0) {
                g_iAnimTimeout = llGetUnixTime() + 20;
            }
        } else if (g_sSubAnim == "~good") {
            g_iAnimTimeout = llGetUnixTime() + 20;
        } else {
            g_iAnimTimeout = 0;
        }
    ...
    changed(integer iChange) {
        if (iChange & CHANGED_INVENTORY) {
            ...
            if (llGetInventoryKey(CARD2) != g_kCardID2) {
                StartNotecards();
            }
        }
