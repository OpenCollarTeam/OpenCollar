...
list g_lAlive=["oc_settings", "oc_api" ,"oc_core"] ;  
integer Alive(string scriptName){   
    integer iScript = llListFindList(g_lAlive,[scriptName]);
    if(~iScript){
        g_lAlive = llDeleteSubList(g_lAlive, iScript, iScript);
        //g_lAlive+=[scriptName];
    }
    if(llGetListLength(g_lAlive)==0){ 
        return TRUE;
    }
    return FALSE;
}
...
Reboot(){        
    integer i = llGetListLength(g_lAlive);
    while(i--)            
        antiCrash(llList2String(g_lAlive,i));                       
...
default
{
    ...
    timer(){        
        ...
        if(llGetListLength(g_lAlive)==1 && llList2String(g_lAlive,0)=="oc_settings")
            state startup;   
            
        if(g_iVerbosityLevel>=1)
            llOwnerSay("ALIVE check failed for "+llDumpList2String(g_lAlive," & "));
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID){
        ...
        if(iNum == ALIVE){    
            ...
            if(sVar == "oc_settings" && kID!=NULL_KEY) 
                g_sGlobalSettings = (string)kID;                
            if(~llListFindList(g_lAlive, [sVar])) // go through every script in g_lAlive
                if(Alive(sVar)) state startup;    // pass if all gave the ALIVE signal
        }
    }
}
...
state startup
{
    ...
    {                
        g_iStartup=0;
        llMessageLinked(LINK_SET, STARTUP, "ALL", g_sGlobalSettings);
        llSetTimerEvent(60); 
    }    
    timer(){        
        if(g_iVerbosityLevel>=1)
            llOwnerSay("Startup timeout on other scripts feedback.");    
        //llResetScript();    
...
state running
{
    ...
    {          
        llSetTimerEvent(0);
        llMessageLinked(LINK_SET, TIMEOUT_READY, "",llGetScriptName());
        
        // check if any haven't started up yet
        llMessageLinked(LINK_SET, READY, "",g_sGlobalSettings);
    }    
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID){
        ...
        if(iNum>=CMD_OWNER && iNum <= CMD_EVERYONE){
            ...
            llMessageLinked(LINK_SET, STARTUP, sStr, g_sGlobalSettings);
            llSleep(1);                    
