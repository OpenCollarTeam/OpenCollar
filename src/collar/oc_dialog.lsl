...
integer g_bSelectAviMenu; // Added to show URIs in menus june 2015 Otto(garvin.twine)
integer g_bColorMenu;
...
Say(string sMsg, integer bWhisper) {
    sMsg = SubstitudeVars(sMsg);
...
Dialog(key kRecipient, string sPrompt, list lMenuItems, list lUtilityButtons, integer iPage, key kID, integer iWithNums, integer iAuth, string sExtraInfo) {
    ...
    if (g_bSelectAviMenu) { // We have to reduce buttons due to text length limitations we reach with URI
        iMyPageSize = iMyPageSize - 3; // june 2015 Otto(garvin.twine)
        if (iNumItems == 8) {
            iMyPageSize = iMyPageSize - 1;
        } else if (iNumItems == 7) { // Special cases again are 7 or 8 avis where we have to reduce "active" buttons again
    ...
    if (iNumItems > iMyPageSize) {
        ...
        integer iNumPages = (iNumItems-1) / iMyPageSize;
        if (iPage > iNumPages) {
            iPage = 0;
        } else if (iPage < 0) {
            iPage = iNumPages;
        ...
        // Multi page menu
        sPagerPrompt = sPagerPrompt + "\nPage " + (string)(iPage + 1) + "/" + (string)(iNumPages + 1);
    }
    ...
    if (iWithNums) { // Put numbers in front of buttons: "00 Button1", "01 Button2", ...
        ...
        for (iCur = iStart; iCur <= iEnd; iCur++) {
            ...
            if ((key)sButton) {
                ...
                } else if (llGetDisplayName((key)sButton)) {
    ...
    integer iTimeout = llGetUnixTime() + g_iTimeOut;

    // Write entry in tracking list
    g_lMenus += [iChan, kID, iListener, iTimeout, kRecipient, sPrompt, llDumpList2String(lMenuItems, "|"), llDumpList2String(lUtilityButtons, "|"), iPage, iWithNums, iAuth, sExtraInfo];
    //Debug("Made Dialog");
}

integer GetStringBytes(string sStr) {
    sStr = llEscapeURL(sStr);
    integer iLength = llStringLength(sStr);
    list lAtoms = llParseStringKeepNulls(sStr, ["%"], []);
    return iLength - 2 * llGetListLength(lAtoms) + 2;
}
...
list PrettyButtons(list lOptions, list lUtilityButtons, list lPageButtons) { // Returns a list formatted to that "options" will start in the top left of a dialog, and "utilitybuttons" will start in the bottom right
    list lSpacers;
    ...
    integer iUpmenuIndex = llListFindList(lCombined, [UPMENU]);
    if (~iUpmenuIndex) {
        lCombined = llDeleteSubList(lCombined, iUpmenuIndex, iUpmenuIndex);
    }
...
CleanList() {
    ...
    for (i = iLength - g_iStrideLength; i >= 0; i -= g_iStrideLength) {
        integer iDieTime = llList2Integer(g_lMenus, i + 3);
        //Debug("dietime: " + (string)iDieTime);
        if (iNow > iDieTime) {
            //Debug("menu timeout");
            key kID = llList2Key(g_lMenus, i + 1);
...
UserCommand(integer iNum, string sStr, key kID) {
    ...
    if (sToken == SPAMSWITCH) { // Add/rem user to verbose=off list
        ...
        if (sValue == "off") {
            ...
            return; // Not in list to start with
        }

        if (!llGetListLength(MRSBUN)) {
            llMessageLinked(LINK_SAVE, LM_SETTING_DELETE, g_sSettingToken + SPAMSWITCH, "");
        } else {
            llMessageLinked(LINK_SAVE, LM_SETTING_SAVE, g_sSettingToken + SPAMSWITCH + "=" + llList2CSV(MRSBUN), "");
        }
    }
}
...
DequeueSensor() {
    ...
    if (llList2Integer(lSensorInfo, 2) == (integer)AGENT) {
        g_bSelectAviMenu = TRUE;
    } else {
        g_bSelectAviMenu = FALSE;
    }
...
default {
    ...
    state_entry() {
        ...
        llSetLinkPrimitiveParamsFast(LINK_THIS, [
            PRIM_NAME, g_sDeviceName
        ]);
        //Debug("Starting");
    }

    sensor(integer iNumDetected){
        ...
        lButtons = [];
        integer i;
        for (; i < iNumDetected; i++) {
            lButtons += llDetectedKey(i);
            if (bReturnFirstMatch || (sFind != "")) { // If we're supposed to be finding the first match,
                if (llSubStringIndex(llToLower(llDetectedName(i)), llToLower(sFind)) == 0 || llSubStringIndex(llToLower(llGetDisplayName(llDetectedKey(i))), llToLower(sFind)) == 0) {
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
        if (iNum == SENSORDIALOG) {
            ...
            list lUButtons = llParseString2List(llList2String(lParams, 4), ["`"], []);
            ...
            ClearUser(kRCPT);
            Dialog(kRCPT, sPrompt, lButtons, lUButtons, iPage, kID, iDigits, iAuth,"");
            ...
            if (sToken == g_sSettingToken + SPAMSWITCH) {
                MRSBUN = llParseString2List(sValue, [","], []);
            } else if (sToken == g_sGlobalToken + "DeviceType") {
    ...
    listen(integer iChan, string sName, key kID, string sMessage) {
        ...
        if (~iMenuIndex) {
            ...
            list lItems = llParseString2List(llList2String(g_lMenus, iMenuIndex + 6), ["|"], []);
            list lUButtons = llParseString2List(llList2String(g_lMenus, iMenuIndex + 7), ["|"], []);
            ...
            if (sMessage == MORE) {
                Dialog(kID, sPrompt, lItems, lUButtons, ++iPage, kMenuID, iDigits, iAuth, sExtraInfo);
                ...
                g_bSelectAviMenu = FALSE;
                string sAnswer;
                integer iIndex = llListFindList(lUButtons, [sMessage]);
                if (iDigits && !~iIndex) {
                    integer iBIndex = (integer) llGetSubString(sMessage, 0, iDigits);
                    sAnswer = llList2String(lItems, iBIndex);
                } else if (g_bColorMenu) {
                    integer iColorIndex  =llListFindList(llList2ListStrided(g_lColors, 0, -1, 2), [sMessage]);
                    if (~iColorIndex) {
                        sAnswer = llList2String(llList2ListStrided(llDeleteSubList(g_lColors, 0, 0), 0, -1, 2), iColorIndex);
                    } else {
                        sAnswer = sMessage;
                    }
                    g_bColorMenu = FALSE;
                ...
                if (sAnswer == "") {
                    sAnswer = " "; // To have an answer to deal with send " "
                }
