        ...
        if (llListFindList(g_lVolatileDestinations, [sCmd]) >= 0) {
            ...
            if (iFound == 0) {
                ...
                */
                ...
                Dialog(kID, "More than one matching bookmark was found in the %DEVICETYPE% of %WEARERNAME%.\nChoose a bookmark to teleport to.", lMatchedBookmarks, [UPMENU], 0, iNum, "choose bookmark");
            } else { // Exactly one matching LM found, so use it
                UserCommand(iNum, PLUGIN_CHAT_CMD + " " + llList2String(lMatchedBookmarks, 0), g_kCommander); // Push matched result to command
            }
...
string FormatRegionName() {
    ...
    string sPosX = (string)llRound(vPos.x);
    string sPosY = (string)llRound(vPos.y);
    string sPosZ = (string)llRound(vPos.z);
...
default {
    ...
    dataserver(key kID, string sData) {
        if (kID == g_kRequestHandle) {
            ...
            vGlobalPos.x = llList2Float(lTokens, 0);
            vGlobalPos.y = llList2Float(lTokens, 1);
            vGlobalPos.z = llList2Float(lTokens, 2);
            vGlobalPos += g_vLocalPos;
            // Build the command
            string sPosStr = (string)((integer)vGlobalPos.x) + "/" + (string)((integer)vGlobalPos.y) + "/" + (string)((integer)vGlobalPos.z);
