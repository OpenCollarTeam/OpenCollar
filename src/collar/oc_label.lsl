...
ShowChars(integer iLink, vector vGrkIDOffset) {
    // SALAHZAR modified .1 to .05 to handle different sized texture
    float fAlpha = llList2Float(llGetLinkPrimitiveParams(iLink, [PRIM_COLOR, FACE]), 1);
    llSetLinkPrimitiveParamsFast(iLink, [
        PRIM_TEXTURE, FACE, (string)g_kFontTexture, g_vRepeats, vGrkIDOffset - g_vOffset, 0.0,
        PRIM_COLOR, FACE, g_vColor, fAlpha
    ]);
}
...
RenderString(integer iLink, string sStr) {
    if (iLink <= 0) {
        return; // Check for negative and zero linknumber
    }
...
SetLabel() {
    ...
    string sPadding;
    if (g_bScroll) {// || llStringLength(g_sLabelText) > g_iCharLimit)
        // Add some blanks
...
SetOffsets(key kFont) {
    // Get 1-st link number from list
    integer iLink = llList2Integer(g_lLabelLinks, 0);
    // Compensate for label box-prims, which must use face 0. Others can be added as needed.
    list lParams = llGetLinkPrimitiveParams(iLink, [PRIM_DESC, PRIM_TYPE]);
    string sDesc = llGetSubString(llList2String(lParams, 0), 0, 4);
    if (sDesc == "Label") {
        integer iType = (integer)llList2String(lParams, 1);
        if (iType == PRIM_TYPE_BOX) {
            if (kFont == NULL_KEY) {
                kFont = "bf2b6c21-e3d7-877b-15dc-ad666b6c14fe"; // LCD default for box
            }
            g_vGridOffset = <-0.45, 0.425, 0.0>;
            g_vRepeats = <0.126, 0.097, 0>;
            g_vOffset = <0.036, 0.028, 0>;
            FACE = 0;
        } else if (iType == PRIM_TYPE_CYLINDER) {
            if (kFont == NULL_KEY) {
                kFont = "2c1e3fa3-9bdb-2537-e50d-2deb6f2fa22c"; // Serif default for cyl
            }
        ...
        integer n = llListFindList(g_lFonts, [(string)kFont]);
    ...
    if (sStr == "rm label") {
        ...
        if (sCommand == "label") {
            if (sAction == "font") {
                ...
                string sFont = llDumpList2String(lParams, " ");
...
default {
    ...
    timer() {
        ...
        g_iSctollPos++;
        if (g_iSctollPos > llStringLength(g_sScrollText)) {
            g_iSctollPos = 0;
        }
    }
    ...
    changed(integer iChange) {
        ...
        if (iChange & CHANGED_COLOR) {
            ...
            if (g_bHide != bNewHide) { // Check there's a difference to avoid infinite loop
                g_bHide = bNewHide;
