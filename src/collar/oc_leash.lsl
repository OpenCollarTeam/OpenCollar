...
key g_kWearer;
integer g_bJustMoved;
// ----- leash -----
integer g_iLength = 3;
integer g_bStay = FALSE;
...
integer g_bPassConfirmed;
integer g_iRezAuth;

string g_sCheck;

// Real-leash variables
integer g_bStrictModeOn = FALSE; // Default is Real-Leash OFF
integer g_bTurnModeOn = FALSE;
integer g_bLeasherInRange = FALSE; //
integer g_bRLVOn = FALSE; // To store if RLV was enabled in the collar
...
ConfirmDialog(key kAv, key kCmdGiver, string sType, integer iAuth) {
    ...
    if (sType == "LeashTarget") {
        ...
        if (kCmdGiver == g_kWearer) {
            sPrompt += "pass you their leash.";
        } else {
            sPrompt += "pass you %WEARERNAME%'s leash.";
        }
        ...
        if (kCmdGiver == g_kWearer) {
            ...
            sPrompt += " command %WEARERNAME% to follow you.";
        }
        sPrompt += "\n\nAre you OK with this?";
...
integer LeashTo(key kTarget, key kCmdGiver, integer iAuth, list lPoints, integer bFollowMode) {
    ...
    */
    ...
    g_bFollowMode = bFollowMode; // Leashing, or following

    if (bTargetIsAvi) {
        g_bLeashedToAvi = TRUE;
    }

    if (llGetOwnerKey(kCmdGiver) == g_kWearer) {
        iAuth = CMD_WEARER; // Prevents owner-wearer with public access creating an unbreakable leash to an unwilling participant
    }

    DoLeash(kTarget, iAuth, lPoints);
    g_bPassConfirmed = FALSE;
...
DoLeash(key kTarget, integer iAuth, list lPoints) {
    ...
    if (g_bFollowMode) {
        ...
        g_sCheck = "";
        if (iPointCount) { // Uf more than one leashpoint, listen for all strings, else listen just for that point
            if (iPointCount == 1) {
                g_sCheck = (string)llGetOwnerKey(kTarget) + llList2String(lPoints, 0) + " ok";
            }
        }
...
Unleash(key kCmdGiver) {
    ...
    if ((key)g_kLeashedTo) {
        ...
        if (bCmdGiverIsAvi) {
            ...
            if (kCmdGiver == g_kWearer) { // Wearer is Leasher
                ...
                if (g_bLeashedToAvi) {
                    llMessageLinked(LINK_DIALOG, NOTIFY, "0" + sTargetMess, g_kLeashedTo);
                }
    ...
    //else llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "%WEARERNAME% is not leashed.", kCmdGiver);
}
...
YankTo(key kIn) {
    ...
    if (llGetAgentInfo(g_kWearer) & AGENT_SITTING) {
        llMessageLinked(LINK_RLV, RLV_CMD, "unsit=force", "");
    }
    llSleep(2.0);
    llStopMoveToTarget();
}

UserCommand(integer iAuth, string sMessage, key kMessageID, integer bRemenu) {
    //Debug("Got user comand:\niAuth: "+(string)iAuth+"\nsMessage: "+sMessage+"\nkMessageID: "+(string)kMessageID+"\nbRemenu: "+(string)bRemenu);
    if (iAuth == CMD_EVERYONE) {
        if (kMessageID == g_kLeashedTo) {
            sMessage = llToLower(sMessage);
            if (sMessage == "unleash" || sMessage == "unfollow" || (sMessage == "toggleleash" && NULL_KEY != g_kLeashedTo)) {
                Unleash(kMessageID);
            } else if (sMessage == "yank") {
                YankTo(kMessageID);
            }
        }
        ...
        if (sMessage == "leashmenu" || sMessage == "menu leash") {
            ...
            if (kMessageID != g_kWearer) {
                lButtons += "Grab"; // Only if not the wearer.
            } else {
                lButtons += "-";
            }
            ...
            if (kMessageID == g_kLeashedTo) {
                lButtons += "Yank"; // Only if leash holder
            } else {
                lButtons += "-";
            }
            ...
            if (sVal == llToLower(BUTTON_UPMENU)) {
                UserCommand(iAuth, "leash", kMessageID, bRemenu);
            } else if (sVal == "me") {
                g_bPassConfirmed = TRUE;
            ...
            if (llGetInventoryType("Pretty Balloon") == INVENTORY_OBJECT) {
                g_iRezAuth = iAuth;
                llRezObject("Pretty Balloon", llGetPos() + (<0.2, 0.0, 1.2> * llGetRot()), ZERO_VECTOR, llEuler2Rot(<0.0, 0.0, 0.0> * DEG_TO_RAD), 0);
            }
        ...
        } else if ((sMessage == "unstay" || sMessage == "move") && g_bStay) {
            if (iAuth <= g_bStayRank) {
                g_bStay = FALSE;
                llReleaseControls();
            ...
            if (g_bStrictModeOn) {
                llMessageLinked(LINK_DIALOG, NOTIFY, "0" + "Strict leashing is already enabled.", kMessageID);
            } else {
                g_iStrictRank = iAuth;
                g_bStrictModeOn = TRUE;
            ...
            if (!CheckCommandAuth(kMessageID, iAuth)) {
                if (bRemenu) {
                    UserCommand(iAuth, "post", kMessageID, bRemenu);
                }
            }

            if (sVal == llToLower(BUTTON_UPMENU)) {
                UserCommand(iAuth, "menu leash", kMessageID, bRemenu);
            } else if((key)sVal) {
                list lPoints;
                if (llGetListLength(lParam) > 2) {
                    lPoints = llList2List(lParam, 2, -1);
...
default {
    ...
    timer() {
        ...
        integer bIsInSimOrJustOutside = TRUE;
        if (vLeashedToPos == ZERO_VECTOR || vLeashedToPos.x < -25 || vLeashedToPos.x > 280 || vLeashedToPos.y < -25 || vLeashedToPos.y > 280) {
            bIsInSimOrJustOutside = FALSE;
        }

        if (bIsInSimOrJustOutside && llVecDist(llGetPos(), vLeashedToPos) < 60) { // If the leasher is now in range
            if(!g_bLeasherInRange) { // And the leasher was previously not in range
                if (g_iAwayCounter) {
                    g_iAwayCounter = 0;
                    llSetTimerEvent(3.0);
                }
            ...
            if(g_bLeasherInRange) { // But was a short while ago
                if (g_iAwayCounter > 3) {
                    llTargetRemove(g_iTargetHandle);
                    llStopMoveToTarget();
                    llMessageLinked(LINK_THIS, CMD_PARTICLE, "unleash", g_kLeashedTo);
    ...
    link_message(integer iSender, integer iNum, string sMessage, key kMessageID){
        if (iNum >= CMD_OWNER && iNum <= CMD_EVERYONE) {
            UserCommand(iNum, sMessage, kMessageID, FALSE);
            ...
            if (llGetSubString(sToken, 0, i) == g_sSettingToken) {
                ...
                if (sToken == TOK_DEST) {
                    ...
                    if (!llGetObjectPrimCount(kTarget) && !g_bLeashedToAvi) {
                        DoUnleash(TRUE);
                    }
                    else {
                        DoLeash(kTarget, (integer)llList2String(lParam, 1), lPoints);
                    }
                } else if (sToken == TOK_LENGTH) {
                    SetLength((integer)sValue);
                } else if (sToken == "strict"){
                    list lParam = llParseString2List(llGetSubString(sMessage, iInd + 1, -1), [","], []);
                    g_bStrictModeOn = (integer)sValue;
                    g_iStrictRank = (integer)llList2String(lParam, 1);
                    ApplyRestrictions();
                } else if (sToken == "turn") {
                    g_bTurnModeOn = (integer)sValue;
                }
            }
        } else if (iNum == RLV_ON) {
            g_bRLVOn = TRUE;
            ...
            if (~iMenuIndex) {
                ...
                } else if (sMenu == "SetLength") {
                    UserCommand(iAuth, "length " + sButton, kAV, TRUE);
    ...
    at_target(integer iNum, vector vTarget, vector vMe) {
        ...
        if (g_bJustMoved) {
            ...
            float fTurnAngle = llAtan2(vPointTo.x, vPointTo.y); // - myAngle;
            if (g_bTurnModeOn) {
                llMessageLinked(LINK_RLV, RLV_CMD, "setrot:" + (string)fTurnAngle + "=force", NULL_KEY); // Transient command, doesn;t need our fakekey
            }
            g_bJustMoved = FALSE;
        }
    }
    ...
    not_at_target() {
        ...
        if(g_kLeashedTo) {
            ...
            if (g_vPos != ZERO_VECTOR) {
                vector vCurrentPos = llGetPos();
                g_vPos = <g_vPos.x, g_vPos.y, vCurrentPos.z>;
