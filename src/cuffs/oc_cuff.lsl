...
SetParticles(integer link, key kID,key kTexture, float fMaxAge, float fGravity){
    ...
    if(fGravity == -1.111) fGravity = -0.076;
...
PosesMenu (key kAv, integer iAuth, integer iPage)
{
    ...
    list lButtons = StrideOfList(g_lPoseMap,  6,0,-1);
...
StartCuffPose(list lParams, integer iSave)
{
    ...
    for(i=0;i<end;i++){
        ...
        Link("from_addon", SUMMON_PARTICLES, llList2String(tmp,0)+"|"+llList2String(tmp,1)+"|"+llList2String(lParams,4)+"|"+llList2String(lParams,3), "");
    }
}
            ...
            else if(sPacketType == "from_collar")
            {
                ...
                if (llVecDist(llGetPos(), llList2Vector(llGetObjectDetails(id, [OBJECT_POS]), 0)) <= 10.0)
                {
                    ...
                    if (iNum == LM_SETTING_RESPONSE)
                    {
                        ...
                        if (sToken == "occuffs")
                        {
                            ...
                            } else if(sVar == g_sPoseName+"pose")
                            {
                                ...
                                g_sCurrentPose=sVal;
                                integer index=llListFindList(g_lPoseMap, [sVal]);
                                if(index!=-1)StartCuffPose(llList2List(g_lPoseMap, index,index+5), FALSE);
                            }
                    ...
                    } else if(iNum == CLEAR_ALL_CHAINS)
                    {
                        ...
                        if(sStr!="NONE"){
                            integer index=llListFindList(g_lPoseMap, [sStr]);
