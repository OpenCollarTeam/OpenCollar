...
integer g_nCmdChannel = -190890; // command channel. This is our private channel built from our UUID
integer g_nCuffChannel = -190889; // cuff channel used for ChainIt chains from the cuffs. This is our private channel +1 built from our UUID
...
SendCmd( string szSendTo, string szCmd, key keyID ) //format the command string to send to slave cuffs
{
    SendCMD (g_szModToken + "|" + szSendTo + "|" + szCmd + "|" + (string)keyID);
}

SendCMD(string szCmd) // Send to slave cuffs
{
...
ParseSingleCmd( key keyID, string szMsg )
{
    ...
    if ( szCmd == "chain" )
    {
        ...
        llMessageLinked(LINK_SET, COMMAND_NOAUTH, "menu", sztoucher);//send to auth
    lstParsed = [];
}
    ...
    if (g_nRLVMode)
    {
        if(g_nArmAnimRunning)
        {
            if (!g_nRLVArms)
            {
                ...
                llMessageLinked(LINK_SET, RLV_CMD, rlvarms, "");
            }
        }
        else
        {
        ...
        if(g_nLegAnimRunning)
        {
            if (!g_nRLVLegs)
            {
                ...
                llMessageLinked(LINK_SET, RLV_CMD, rlvlegs, "");
            }
        }
        else
        {
        ...
        if(g_nWingAnimRunning)
        {
            if (!g_nRLVWings)
            {
                ...
                g_nRLVWings=TRUE;
                rlvwings = "fly=n";
                llMessageLinked(LINK_SET, RLV_CMD, rlvwings, "");
            }
        }
         else
        {
...
//                if (ShowMessages) llOwnerSay("Your Tail is bound, so you can only limited move."); //place holder
                g_nRLVTail=TRUE;
                rlvtail = "";
//                llMessageLinked(LINK_SET, RLV_CMD, rlvtail, ""); //place holder
            }
        }
         else
        {
...
//        llOwnerSay((string)nNum + " : " +szCmd);
        if (nNum == 6101)
            SendCmd("*","rlvon","");
        else if (nNum == 6100)
            SendCmd("*","rlvoff","");
        else if (szCmd == "hide" || szCmd == "show")
            SendCmd("*",szMsg,"");
    ...
    listen(integer nChannel, string szName, key keyID, string szMsg)
    {
        ...
        if ( nChannel == g_nCmdChannel )// commands sent on cmd channel, they should be from slave cuffs
        {
            if ( llGetOwnerKey(keyID) == g_keyWearer )
            { // check if external or maybe for this module
                string szCmd = CheckCmd( szMsg );
                if ( g_nCmdType == CMD_MODULE )
                    llMessageLinked(LINK_THIS, LM_CUFF_CMD, szCmd, llGetOwnerKey(keyID));
            }
        }
        else if ( nChannel == g_nStdChannel )//ChainIt chain commands some in here
