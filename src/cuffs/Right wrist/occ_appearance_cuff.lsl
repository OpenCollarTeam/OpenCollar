...
SendDefChainCommand()
{
    ...
    if ((ChainCurrent>=0) && (ChainCurrent<llGetListLength(ChainMenuButtons)))
        ...
        s="";
    llMessageLinked(LINK_THIS,LM_CUFF_CHAINTEXTURE,s,"");
}
...
default
{
    state_entry()
    {
        ...
        llSleep (2.0);
        llMessageLinked(LINK_SET, LM_SETTING_REQUEST, chaintoken , "");
    }
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        else if (iNum >= COMMAND_OWNER && iNum <= COMMAND_WEARER)
        {
            ...
            {
                SendDefChainCommand();
                llMessageLinked(LINK_SET,COMMAND_NOAUTH,"resend_appearance", kID);
            }
        }
