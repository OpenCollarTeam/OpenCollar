    ...
    if (~llListFindList(commands, [llList2String(lParams,0)]) || (llList2String(lParams,0)=="menu" && ~llListFindList(commands, [llList2String(lParams,1)])) ) {  //this is for us....
        if (kID == g_kWearer || iNum == CMD_OWNER) {  //only allowed users can...
            ...
            if (sCommand == "themes" || sStrLower == "menu themes") {
                ...
                llMessageLinked( LINK_THIS, 551, "rlac|*|" + sCommand + "|" + (string)kID, kID);//send to all other cuffs the hide/show
                ...
                if (sShiny=="") ShinyMenu(kID, iNum, sStr);
                    ...
                    llMessageLinked( LINK_THIS, 551, "rlac|*|" + "shiny_" + sElement + "=" + (string)iShiny + "|" + (string)kID, kID);//send to all other cuffs
                ...
                if (sGlow=="") {  //got no value for glow, make glow menu
                    ...
                    llMessageLinked( LINK_THIS, 551, "rlac|*|" + "glow_" + sElement + "=" + (string)fGlow + "|" + (string)kID, kID);//send to all other cuffs
                ...
                if (sColor != "") {
                    ...
                    llMessageLinked( LINK_THIS, 551, "rlac|*|" + "color_" + sElement + "=" + sColor + "|" + (string)kID, kID);//send to all other cuffs
                ...
                if (sTextureShortName=="") {  //no texture name supplied, send texture menu for this element
                    ...
                    llMessageLinked(LINK_THIS, 551, "rlac|*|" + "texture_" + sElement + "=" + sTextureKey + "|" + (string)kID, kID);//send to all other cuffs
                    if (reMenu) TextureMenu(kID, 0, iNum, sCommand+" "+sElement);
                }
            }
...
default {
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
        if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) UserCommand(iNum, sStr, kID, FALSE);
            ...
            if (sStr == "show" || sStr == "hide")
                UserCommand(500,sStr,kID, FALSE);//NG added
