...
integer LoadLocksParse( key queryid, string data)
{
    ...
    if ( data == EOF )
    {
        ...
        llMessageLinked(LINK_SET, LM_SETTING_REQUEST, dbtoken, "");//lets make sure we get our pose sync back once notecard is read
...
CallAnim( string szMsg, key keyID )
{
    ...
    {
        g_szActAnim = "";
        llMessageLinked( LINK_THIS, LM_CUFF_ANIM, "c:Stop", keyID );
    }
    else
    {
    ...
    { //owner, secowner, group, and wearer may currently change colors
        ...
        else if ( nNum == DIALOG_RESPONSE)
        {
            if (id==g_keyDialogID)
            {
                ...
                {
                    Collar_Chains = TRUE;
                    llMessageLinked(LINK_SET, LM_SETTING_SAVE, dbtoken +"="+ (string)Collar_Chains, "");
                    buttons = ["Off"];
                    DoMenu(id);
                }
                else if (message =="Off")
