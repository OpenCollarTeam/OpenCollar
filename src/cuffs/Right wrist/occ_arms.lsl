...
integer LoadLocksParse( key queryid, string data)
{
    ...
    if ( data == EOF )
    {
        ...
        llMessageLinked(LINK_SET, LM_SETTING_REQUEST, dbtoken, "");//lets make sure we get our pose back once notecard is read
        return -1;
    }
    pos_line ++;
    LoadLocksNextLine();
...
{
    llMessageLinked( LINK_THIS, SENDCMD, llList2String(g_lstModTokens,0) + "|" + szSendTo + "|" + szCmd + "|" + (string)keyID, keyID);
}
...
Chains(key keyID, string szChain, string szLink)
{
    ...
    if ( llListFindList(g_lstModTokens,[szTo]) != -1 )
        llMessageLinked( LINK_THIS, LM_CUFF_CMD, "chain=" + szChain + "=" + szCmd, llGetKey() );
    else
...
CallAnim( string szMsg, key keyID )
{
    ...
    {
        g_szActAnim = "";
        llMessageLinked( LINK_THIS, LM_CUFF_ANIM, "a:Stop", keyID );
    }
    else
    {
        ...
        if (nIdx != -1 )
        {
            ...
            if (szAnim=="*none*") // Cleo: We stop any maybe running arm anim, as on space we dont want anims
                ...
                llMessageLinked( LINK_THIS, LM_CUFF_ANIM, "a:"+szAnim, keyID );
        ...
        else if (nNum >= COMMAND_OWNER && nNum <= COMMAND_WEARER)
        {
            if ( startswith(str,"*:") || startswith(str,"a:") )
            {
                if (nNum <= lastrank)
                {
                    ...
                    llMessageLinked(LINK_SET, LM_SETTING_SAVE, dbtoken +"="+ llGetSubString(str, 2,-1), "");
                }
                else
